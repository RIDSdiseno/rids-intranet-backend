...
import type { SiiApiRequestParams } from "../types/sii-api.types.js";

const SIMPLEAPI_BASE_URL =
    process.env.SIMPLEAPI_BASE_URL ?? "https://servicios.simpleapi.cl";

export async function siiApiRequest<T>({
    empresaKey,
    empresaConfig,
    endpoint,
    method = "GET",
    params,
}: SiiApiRequestParams): Promise<T> {
    ...
    try {
        ...
        if (!ok) {
            throw new Error(
                `SimpleAPI ${response.status}: ${JSON.stringify(response.data)}`
            );
        }
