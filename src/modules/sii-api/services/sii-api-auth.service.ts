import type { EmpresaKey, EmpresaConfig } from "../types/sii-api.types.js";

// --- SimpleAPI Key ---
export function getSimpleApiKey(): string {
    const key = process.env.SIMPLEAPI_KEY;
    if (!key) throw new Error("SIMPLEAPI_KEY no está definida en .env");
    return key;
}

// --- Credenciales SII por empresa ---
const EMPRESA_CONFIGS: Record<EmpresaKey, () => EmpresaConfig> = {
    econnet: () => {
...
export function getEmpresaConfig(empresaKey: EmpresaKey): EmpresaConfig {
    const resolver = EMPRESA_CONFIGS[empresaKey];
    if (!resolver) throw new Error(`Empresa desconocida: ${empresaKey}`);
    return resolver();
}