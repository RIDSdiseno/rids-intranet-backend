...
export type RunAIInput = {
  ...
  context?: {
    ...
    company?: string;
    name?: string;
    phone?: string;           // para guardar en Freshdesk (custom field), opcional
      ...
      parameters: {
        ...
        properties: {
          ...
          transcript: {
            type: "array",
            description:
              "Conversación reciente para adjuntar al ticket (ordenada)",
            items: {
              type: "object",
              properties: {
...
async function callOpenAI(messages: ChatMessage[]) {
  if (!OPENAI_API_KEY) {
    return {
      text:
        "Hola 👋 Soy RIDSI. Cuéntame en qué te puedo ayudar en ventas o soporte, y si quieres generar un ticket luego, necesitaré tu correo y el nombre de tu empresa.",
