...
const localPart = (email?: string | null): string | null => {
  if (!email) return null;
  const at = email.indexOf("@");
...
async function upsertTicketRequesterByAny(params: {
  ticketOrgId: number | null;
  fdRequesterId?: number | string | bigint | null;
  email?: string | null;
  phone?: string | null;
  ...
  if (rid) {
    ...
    if (foundByRid) {
      ...
      if (name && name !== foundByRid.name) updateData.name = name;
  ...
  if (emailNorm) {
    ...
    if (byEmail) {
      const updateData: any = {};
      if (phone && !byEmail.phone) updateData.phone = phone;
      if (name && name !== byEmail.name) updateData.name = name;
...
export async function upsertTicketBatch(tickets: TicketFD[]) {
  for (const t of tickets) {
    ...
    if (typeof ticketId !== "bigint") continue;
