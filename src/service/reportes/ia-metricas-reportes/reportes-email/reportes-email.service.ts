...
function validateEmailList(emails: string[], fieldName: string) {
    ...
    if (invalidos.length > 0) {
        throw new Error(
            `Correos inválidos en ${fieldName}: ${invalidos.join(", ")}`
        );
    }
}
...
function buildInformeResumenEmailHtml(params: EnviarInformeResumenEmailParams) {
    const mensajeSeguro = escapeHtml(
        params.mensaje?.trim() ||
        "Se adjunta el informe generado desde el sistema."
    ).replace(/\n/g, "<br>");
...
export async function enviarInformeResumenPorCorreo(
    params: EnviarInformeResumenEmailParams
) {
