...
function obtenerFechaLocalChile(fecha: Date | string): string {
...
function calcularTotalMinutosPorJornada(
    visitas: Array<{
        inicio: Date | string | null;
        fin: Date | string | null;
    ...
    for (const jornada of jornadas.values()) {
        const minutos = Math.round(
            (jornada.finMax - jornada.inicioMin) / 1000 / 60
        );
...
===================== */
    ...
    type LicenciaUsuarioNormalizada = {
    ...
    const licenciasUnicasPorUsuarioSku = Array.from(
        licenciasUnicasPorUsuarioSkuMap.values()
    );
    ...
    const usuariosConLicencias = Array.from(
        usuariosUnicosConLicenciaMap.values()
    ...
    const avgMinutos = duracionesMinutosVisitas.length
        ? Math.round(
            duracionesMinutosVisitas.reduce((a, b) => a + b, 0) /
    ...
    const inventarioPorMarcaMap = new Map<string, number>();

    for (const equipo of equipos) {
        const marca = equipo.marca?.trim() || "Sin marca";
        inventarioPorMarcaMap.set(
            marca,
            (inventarioPorMarcaMap.get(marca) ?? 0) + 1
        );
    }
 ...
 ===================== */
    ...
    const ticketsDetalle: any[] = [
        ...ticketsFreshdeskRaw.map((t) => ({
            ...
            ticketRequester: {
                name:
                    t.ticketRequester?.name ??
                    t.solicitante?.nombre ??
                    "Sin nombre",
                email:
                    t.ticketRequester?.email ??
                    t.solicitante?.email ??
                    null,
            },
        })),
    ...
    const topUsuariosGeneral =
        obtenerTopUsuariosGeneral(visitasDetalle, ticketsDetalle);
    ...
    const narrativa = {
        resumen:
            `Durante el periodo ${ym}, se realizaron ${visitasCount} visitas técnicas con una duración promedio de ${avgTiempoTexto} por intervención.`,
    };
