...
export type CandidatoFacturaEmitida = {
    ...
    empresaClienteActiva:
    boolean;

    empresaRecibeFacturas:
    boolean;
...
function calcularDiasAntiguedad(
    fechaIso: string,
    referencia =
        new Date()
): number | null {
...
function getPeriodosNecesarios(
...
async function cargarFacturasRcv(
    empresa:
        EmpresaFacturaKey,
    ...
    console.log(
        `[FACTURA AUTO] ${empresa.toUpperCase()} períodos a consultar`,
        periodos.map(
            (
                item
            ) =>
    ...
    for (
        ...
        try {
            ...
            const diferenciaMeses =
                periodoActualNumerico -
                periodoNumerico;

            const esPeriodoReciente =
                diferenciaMeses >=
                0 &&
                diferenciaMeses <=
                1;
            ...
            console.log(
                `[FACTURA AUTO] ${empresa.toUpperCase()} consultando período`,
                {
                    periodo:
                        periodo.periodo,
            ...
            console.log(
                `[FACTURA AUTO] ✅ ${empresa.toUpperCase()} ${periodo.periodo}`,
                {
                    documentos:
                        docs.length,
...
async function prepararReceptoresFacturacionDesdeRcv(
    documentos: any[]
) {
    /*
...
async function cargarReceptoresFacturacion(
    ruts:
        string[]
) {
    const rutsUnicos =
...
> {
    console.log(
        `[FACTURA AUTO] ▶ Iniciando ${empresa.toUpperCase()}`
    );
    ...
    if (
        ...
        return {
            ...
            error:
                "No existe RcvFacturaEnvioConfig",
        };
    }
    ...
    if (
        ...
        await prepararReceptoresFacturacionDesdeRcv(
            documentos
        );
    }

    const rutsDocumentos =
        documentos
            .map(
                (
                    documento
    ...
    const receptoresPorRut =
        await cargarReceptoresFacturacion(
            rutsDocumentos
        );
    ...
    const enviosMap =
        new Map<
            string,
            (typeof enviosExistentes)[number]
        >();
    ...
    let facturasTipoPermitido =
        0;

    let fueraDeAntiguedad =
        0;
    ...
    console.log(
        `[FACTURA AUTO] ${empresa.toUpperCase()} resumen`,
        {
            documentosAnalizados:
                documentos.length,
...
function construirAsuntoFactura(
    params: {
        empresa:
        EmpresaFacturaKey;
    ...
    return (
        `${nombreDocumento} N° ${params.folio} - ${emisor}`
    );
}

async function ejecutarAnalisisFacturas(
    options?: {
        empresas?:
        EmpresaFacturaKey[];
...
export async function simularFacturasEmitidas(
    options?: {
        empresas?:
        EmpresaFacturaKey[];
    }
...
> {
    ...
    console.log(
        "[FACTURA AUTO] INICIO SIMULACIÓN"
    );

    const resultado =
        await ejecutarAnalisisFacturas({
            ...(options?.empresas
                ? {
                    empresas:
                        options.empresas,
...
export async function prepararFacturasEmitidas(
    options?: {
        empresas?:
        EmpresaFacturaKey[];
         ...
         *
         * true:
         *   refresca mes actual + mes anterior
         *   desde BaseAPI/SII.
         *
         * El scheduler debe utilizar true.
         */
        refrescarPeriodosRecientes?:
        boolean;
    }
): Promise<
    ResultadoPreparacionFacturas
> {
    console.log(
        "[FACTURA AUTO] ========================================"
    );
    ...
    console.log(
        "[FACTURA AUTO] INICIO PREPARACIÓN"
    );
    ...
    let candidatosEnviables =
        0;

    for (
        const resultadoEmpresa
        of simulacion.empresas
    ) {
        if (
            !resultadoEmpresa
                .configurada ||
     ...
     */
    const registrosUnicosMap =
        new Map<
            string,
            (
                typeof registrosPreparar
            )[number]
        >();

    for (
        const registro
        of registrosPreparar
    ) {
        const key =
            [
                registro
    ...
    console.log(
        "[FACTURA AUTO] PREPARACIÓN FINALIZADA",
        {
            documentosEvaluados:
                simulacion
                    .totalCandidatos,
