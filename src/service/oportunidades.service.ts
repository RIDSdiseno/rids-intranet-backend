...
export class CotizacionOportunidadRequeridaError extends Error {
  constructor(
    message = "Debe preparar y enviar una cotización antes de avanzar a negociación."
  ) {
    super(message);
...
export class DesvinculacionCotizacionInvalidaError extends Error {
  constructor(
    message = "No se puede desvincular la cotización sin dejar la oportunidad en un estado inválido."
  ) {
    super(message);
...
/* ======================================================
   Tipos de entrada (la validación Zod vive en el controller;
   estos tipos describen lo que el service espera recibir ya validado)
====================================================== */

// NOTA: con `exactOptionalPropertyTypes` habilitado, un campo opcional definido
// como `field?: T` NO acepta que la propiedad esté presente con valor `undefined`
// (que es justamente lo que produce z.optional() al inferir tipos). Por eso los
// campos opcionales se tipan explícitamente como `T | undefined`.
...
const ETAPAS_SIN_COTIZACION_REQUERIDA: ReadonlySet<EtapaOportunidadVenta> = new Set([
...
const ETAPAS_QUE_REQUIEREN_COTIZACION_VINCULADA: ReadonlySet<EtapaOportunidadVenta> = new Set([
  EtapaOportunidadVenta.COTIZACION_ENVIADA,
  EtapaOportunidadVenta.NEGOCIACION,
  EtapaOportunidadVenta.GANADA,
]);

// El cliente `prisma` está extendido ($extends) para auditoría automática,
// por lo que Prisma.TransactionClient no calza con el tipo real de `tx`
// dentro de prisma.$transaction(async (tx) => ...). Mismo patrón usado en
...
========================================================= */
async function generarCodigoOportunidad(tx: TxClient): Promise<string> {
  ...
  if (ultimo?.codigo) {
    ...
    if (Number.isFinite(parsed)) siguiente = parsed + 1;
  }
...
========================================================= */
async function validarCotizacionParaEtapa(
  tx: TxClient,
  oportunidadId: number,
  etapaDestino: EtapaOportunidadVenta
) {
  if (!ETAPAS_QUE_REQUIEREN_COTIZACION_VINCULADA.has(etapaDestino)) return;
...
========================================================= */
async function recalcularOrdenColumna(
  tx: TxClient,
  etapa: EtapaOportunidadVenta,
  idMovida: number,
...
async function siguienteOrdenEtapa(tx: TxClient, etapa: EtapaOportunidadVenta): Promise<number> {
...
/* =========================================================
   Crear oportunidad
========================================================= */
export async function crearOportunidad(actorId: number, payload: CrearOportunidadInput) {
  ...
  if (!responsable) throw new ResponsableOportunidadInvalidoError();
  ...
  if (payload.entidadId) {
    ...
    if (!entidad) throw new EntidadOportunidadInvalidaError();
  }
  ...
  while (true) {
    ...
    try {
      ...
      if (isUniqueConstraintError(err) && intento < MAX_INTENTOS) continue;
      throw err;
    }
  }
}
...
========================================================= */
export async function listarOportunidades(filtros: FiltrosOportunidadInput) {
  const where: Prisma.OportunidadVentaWhereInput = { activo: true };
...
========================================================= */
const ETAPAS_ACTIVAS: ReadonlySet<EtapaOportunidadVenta> = new Set([
  EtapaOportunidadVenta.NUEVA,
  EtapaOportunidadVenta.COTIZACION_PREPARACION,
  EtapaOportunidadVenta.COTIZACION_ENVIADA,
  EtapaOportunidadVenta.NEGOCIACION,
...
const ETAPAS_FUNNEL_CONVERSION: EtapaOportunidadVenta[] = [
  EtapaOportunidadVenta.NUEVA,
  EtapaOportunidadVenta.COTIZACION_PREPARACION,
  EtapaOportunidadVenta.COTIZACION_ENVIADA,
  EtapaOportunidadVenta.NEGOCIACION,
...
export async function obtenerDashboardFunnel(filtros: FiltrosDashboardInput) {
  const where: Prisma.OportunidadVentaWhereInput = { activo: true };
  ...
  if (filtros.entidadId) where.entidadId = filtros.entidadId;
  if (filtros.origen) where.entidad = { origen: filtros.origen };
  if (filtros.texto) {
    where.OR = [
      { titulo: { contains: filtros.texto, mode: "insensitive" } },
      { proyecto: { contains: filtros.texto, mode: "insensitive" } },
  ...
  const accionesVencidasActivas = activas.filter(
    (o) => o.fechaProximaAccion != null && o.fechaProximaAccion < inicioHoy
  );

  const proximasAcciones = {
    vencidas: accionesVencidasActivas.length,
    hoy: activas.filter(
      (o) => o.fechaProximaAccion != null && o.fechaProximaAccion >= inicioHoy && o.fechaProximaAccion <= finHoy
    ).length,
    proximos7: activas.filter((o) => {
      if (!o.fechaProximaAccion) return false;
  ...
  const porEtapaMap = new Map<EtapaOportunidadVenta, { cantidad: number; monto: number }>();
  ...
  function agruparPorMonto<T>(items: typeof activas, clave: (o: (typeof activas)[number]) => T) {
    const mapa = new Map<T, { cantidad: number; monto: number }>();
  ...
  function calcularForecast(dias: number) {
    const limite = finDeDia(new Date(inicioHoy.getTime() + dias * 24 * 60 * 60 * 1000));
    const enRango = activas.filter(
      (o) => o.fechaProbableCierre != null && o.fechaProbableCierre >= inicioHoy && o.fechaProbableCierre <= limite
    );
    const montoTotal = enRango.reduce((acc, o) => acc + numero(o.montoEstimado), 0);
    const montoPonderado = enRango.reduce(
      (acc, o) => acc + numero(o.montoEstimado) * ((o.probabilidadCierre ?? 50) / 100),
      0
    );
  ...
  function agruparMotivos(items: typeof perdidas, extractor: (o: (typeof perdidas)[number]) => string | null) {
  ...
  const cantidadPorEtapaFunnel = ETAPAS_FUNNEL_CONVERSION.map(
    (etapa) => porEtapaMap.get(etapa)?.cantidad ?? 0
  );
...
/* =========================================================
   Detalle completo
========================================================= */
export async function obtenerOportunidadPorId(id: number) {
...
async function obtenerOportunidadActivaOrThrow(tx: TxClient, id: number) {
...
========================================================= */
export async function editarOportunidad(id: number, payload: EditarOportunidadInput) {
...
export async function cambiarEtapaOportunidad(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const data: Prisma.OportunidadVentaUpdateInput = { etapa: etapaNueva };
...
========================================================= */
export async function reordenarOportunidad(id: number, nuevoOrden: number) {
  if (nuevoOrden < 0) throw new OrdenOportunidadInvalidoError();
...
/* =========================================================
   Soft delete
========================================================= */
export async function desactivarOportunidad(id: number) {
  return prisma.$transaction(async (tx) => {
    await obtenerOportunidadActivaOrThrow(tx, id);
    return tx.oportunidadVenta.update({
      where: { id },
...
export async function listarSeguimientos(oportunidadId: number) {
  const existe = await prisma.oportunidadVenta.findUnique({ where: { id: oportunidadId }, select: { id: true } });
  if (!existe) throw new OportunidadNoEncontradaError();
...
========================================================= */
export async function obtenerHistorial(oportunidadId: number) {
  const existe = await prisma.oportunidadVenta.findUnique({ where: { id: oportunidadId }, select: { id: true } });
  if (!existe) throw new OportunidadNoEncontradaError();
...
========================================================= */
export async function listarCotizacionesOportunidad(oportunidadId: number) {
  const existe = await prisma.oportunidadVenta.findUnique({ where: { id: oportunidadId }, select: { id: true } });
  if (!existe) throw new OportunidadNoEncontradaError();
...
export async function vincularCotizacion(oportunidadId: number, cotizacionId: number) {
  return prisma.$transaction(async (tx) => {
    await obtenerOportunidadActivaOrThrow(tx, oportunidadId);
    ...
    if (!cotizacion) throw new CotizacionNoEncontradaError();
...
export async function desvincularCotizacion(oportunidadId: number, cotizacionId: number) {
  return prisma.$transaction(async (tx) => {
...
========================================================= */
export async function sincronizarOportunidadPorCotizacionAprobada(
  cotizacionId: number,
  actorId: number | null
) {
  try {
    ...
    if (!cotizacion?.oportunidadVentaId) return;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (!ETAPAS_SIN_COTIZACION_REQUERIDA.has(oportunidad.etapa)) return; // no retrocede etapas avanzadas
    ...
    console.error(
      "[oportunidades] Error sincronizando oportunidad tras aprobar cotización:",
      err
    );
  }
}
