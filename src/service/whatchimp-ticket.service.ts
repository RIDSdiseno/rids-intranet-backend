...
export async function searchEmpresaByName(
  ...
  const aliasHits = new Set(
    allAliases.filter(a => matches(a.alias)).map(a => a.empresaId)
  );
...
export async function createTicketFromWhatsapp(
  ...
  try {
    ...
    let empresa: { id_empresa: number; nombre: string } | null = null;
    ...
    if (email.includes("@")) {
      ...
      if (empresa) console.log(`[WC-TICKET] Empresa por dominio "${domain}": ${empresa.nombre}`);
    }
