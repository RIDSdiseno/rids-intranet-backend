...
export async function syncTeamViewer(req: Request, res: Response) {
    try {
        ...
        // ⚠ sumar 1 segundo para evitar duplicado por milisegundos
        const fromDate = lastSession?.inicio
            ? new Date(lastSession.inicio.getTime() + 1000)
                .toISOString()
                .replace(/\.\d{3}Z$/, "Z") // 🔥 elimina milisegundos
            : undefined;
        ...
        if (!sessions.length) {
            return res.json({ ok: true, totalRecibidas: 0 });
        }
        ...
        for (const session of sessions) {
            ...
            if (!empresaId && deviceId) {
                try {
                    ...
                    const solAlias = solicitantes.find(s =>
                        alias.includes(s.nombre.toLowerCase())
                    );
