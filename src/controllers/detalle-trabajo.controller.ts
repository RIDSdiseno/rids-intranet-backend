    ...
    try {
        ...
        const nuevo = await prisma.detalleTrabajo.create({
            ...
            include: {
                ...
                equipo: { select: { id_equipo: true, serial: true, marca: true, modelo: true } },
...
export async function updateDetalleTrabajo(req: Request, res: Response) {
    try {
        ...
        if (parsed.fecha_ingreso !== undefined) data.fecha_ingreso = new Date(parsed.fecha_ingreso);
        ...
        if (parsed.fecha_prometida !== undefined)
            data.fecha_prometida = parsed.fecha_prometida ? new Date(parsed.fecha_prometida) : null;
