        ...
        if (!isInternal) {
            ...
            if (tecnicoDestino) {
                try {
                    ...
                    console.error(
                        "⚠️ Error cancelando recordatorio automático al responder ticket:",
                        err
                    );
                }
            }
...
export async function updateTicket(req: Request, res: Response) {
    try {
        ...
        const isTryingToAssign =
            assigneeId !== undefined && assigneeId !== ticket.assigneeId;
        ...
        if (status === TicketStatus.PENDING && status !== ticket.status) {
            void sendTicketPendingEmail(ticketId).catch((err) => {
                console.error(
                    "⚠️ Error enviando correo de ticket pendiente:",
                    err
                );
            });
            ...
            if (tecnicoDestino) {
                ...
                try {
                    ...
                    console.error(
                        "⚠️ Error creando recordatorio automático de ticket pendiente:",
                        err
                    );
                }
            }
        ...
        if (
            ...
            if (tecnicoDestino) {
                try {
                    ...
                    console.error(
                        "⚠️ Error cancelando recordatorio automático de ticket:",
                        err
                    );
                }
            }
            ...
            void Promise.allSettled(
                pendingTicketIds.map(async (ticketId) => {
                    try {
                         ...
                         */
                        console.error(
                            `⚠️ Error enviando correo de pendiente para ticket #${ticketId}:`,
                            error
                        );
 ...
 */
        if (status === TicketStatus.PENDING) {
            const ticketsPendientesParaRecordatorio = ticketsBefore.filter(
                (ticket): ticket is typeof ticket & { assigneeId: number } =>
                    ticket.status !== TicketStatus.PENDING &&
                    typeof ticket.assigneeId === "number"
            );

            await Promise.allSettled(
                ticketsPendientesParaRecordatorio.map(async (ticket) => {
                    const recordatorioAt = new Date(
                        Date.now() + 60 * 60 * 1000
                    );
                    ...
                    try {
                        ...
                        console.error(
                            `⚠️ Error creando recordatorio automático para ticket #${ticket.id}:`,
                            error
                        );
 ...
 */
        if (status && status !== TicketStatus.PENDING) {
            const ticketsQueSalenDePendiente = ticketsBefore.filter(
                (ticket): ticket is typeof ticket & { assigneeId: number } =>
                    ticket.status === TicketStatus.PENDING &&
                    typeof ticket.assigneeId === "number"
            );
            ...
            await Promise.allSettled(
                ticketsQueSalenDePendiente.map(async (ticket) => {
                    try {
                        await sincronizarRecordatorioTicket({
                            ticketId: ticket.id,
                            tecnicoId: ticket.assigneeId,
                        ...
                        console.error(
                            `⚠️ Error cancelando recordatorio automático para ticket #${ticket.id}:`,
                            error
                        );
