...
import { getSlaConfigFromDB, type Priority } from "../../../config/sla.config.js";

const PRIORITY_ORDER: Priority[] = ["LOW", "NORMAL", "HIGH", "URGENT"];

export async function getSlaConfig(req: Request, res: Response) {
    try {
...
export async function updateSlaConfig(req: Request, res: Response) {
    try {
        ...
        if (
            typeof firstResponseMinutes !== "number" || firstResponseMinutes < 1 ||
