...
function buildDateFilter(
    ...
    let sql = "";
    let idx = startIdx;
    if (fromDate) { sql += ` AND t."createdAt" >= $${idx++}`; params.push(fromDate); }
    if (toDate) { sql += ` AND t."createdAt" < $${idx++}`; params.push(toDate); }
    return { sql, params, nextIdx: idx };
}

// ── GET /api/helpdesk/tickets-dashboard/monthly ──────────────────────────────
export async function getTicketsDashboardMonthly(req: Request, res: Response) {
    try {
...
export async function getTicketsDashboardRanking(req: Request, res: Response) {
    try {
        const fromDate = typeof req.query.fromDate === "string" ? req.query.fromDate : undefined;
        const toDate = typeof req.query.toDate === "string" ? req.query.toDate : undefined;
