...
const DIAS_EXPIRACION_RECORDATORIO =
    3;
...
async function expirarRecordatoriosPendientes(
    tecnicoId: number
): Promise<number> {
    const ahora =
        new Date();
    ...
    if (
        recordatoriosExpirados.length ===
        0
    ) {
        return 0;
    }

    const recordatorioIds =
        recordatoriosExpirados.map(
            (
                recordatorio
            ) =>
                recordatorio.id
 ...
 */
        await expirarRecordatoriosPendientes(
            tecnicoId
        );
