...
export const getVisitasDashboard = async (req: Request, res: Response) => {
  try {
    ...
    const diasConVisitas = new Set(
      jornadas
        .filter((j) => j.inicio)
        .map((j) => new Date(j.inicio).toISOString().slice(0, 10))
    ).size;
    ...
    const porMesMap = new Map<string, { mes: string; visitas: number; minutos: number }>();
    ...
    for (const j of jornadas) {
      ...
      if (!porMesMap.has(mes)) porMesMap.set(mes, { mes, visitas: 0, minutos: 0 });
    ...
    const porDiaMap = new Map<string, { fecha: string; visitas: number; minutos: number }>();
    ...
    for (const j of jornadas) {
      ...
      if (!porDiaMap.has(fecha)) porDiaMap.set(fecha, { fecha, visitas: 0, minutos: 0 });
    ...
    const porTecnicoFullMap = new Map<string, {
      tecnico: string;
      jornadas: number;
      minutos: number;
      diasUnicos: Set<string>;
      mesesMap: Map<string, { mes: string; jornadas: number; minutos: number; diasUnicos: Set<string> }>;
    }>();
    ...
    const porEmpresaMap = new Map<string, {
      nombre: string;
      jornadas: number;
      minutos: number;
      diasUnicos: Set<string>;
