// src/controllers/controllers-agente-inventario/equipo-agent-dashboard.controller.ts
...
type EstadoAgenteCalculado =
    | "ACTIVO"
    | "SIN_CONEXION"
    | "SIN_AGENTE";

type EstadoOneDriveCalculado =
    | "OPERATIVO"
    | "NO_INSTALADO"
    | "NO_EJECUTANDO"
    | "SIN_USUARIO"
...
/*
 * Resumen de cobertura mensual agrupado por empresa.
 * Esta información se calcula antes de aplicar los filtros
 * de estado del agente y OneDrive.
 */
type CoberturaEmpresaDashboard = {
...
type AgentEventMetadata = {
    ...
    requiereRevisionSolicitante?: boolean | null;
};
...
function parseEventMetadata(
    ...
    return metadata as AgentEventMetadata;
}
...
function getChileMonthRange(
    ...
    const hasta = fromZonedTime(
        `${nextYear}-${nextMonthText}-01T00:00:00`,
        CHILE_TIMEZONE
    );
...
function formatChileDateKey(date: Date): string {
...
function calcularEstadoAgente(
    lastSeenAt: Date | null,
    horasSinConexion: number
): EstadoAgenteCalculado {
    if (!lastSeenAt) {
        return "SIN_AGENTE";
    }
...
function calcularEstadoActualOneDrive(
    detalle: {
...
function calcularSaludMensualOneDrive(
    eventos: Array<{
        metadata: Prisma.JsonValue | null;
    }>
): {
    estado: SaludOneDriveMensual;
    ...
    const snapshots = eventos
        .map((evento) => {
            const metadata =
                parseEventMetadata(evento.metadata);

            const instalado =
                boolOrNull(metadata.oneDriveInstalado);

            const enEjecucion =
                boolOrNull(metadata.oneDriveEnEjecucion);

            const operativo =
                boolOrNull(metadata.oneDriveOperativo);

            const usuario =
                String(
                    metadata.oneDriveUsuario ?? ""
                ).trim();

            const version =
                String(
                    metadata.oneDriveVersion ?? ""
                ).trim();
    ...
    const todosNoInstalados = snapshots.every(
        (snapshot) =>
            snapshot.instalado === false
    );

    const usuariosDetectados = Array.from(
        new Set(
            snapshots
    ...
    const versionesDetectadas = Array.from(
        new Set(
            snapshots
    ...
    const analisisOperativos =
        snapshots.filter(
            (snapshot) =>
                snapshot.operativo === true
        ).length;

    const analisisConFalla =
        snapshots.length - analisisOperativos;

    const porcentajeOperativo = Number(
        (
            (
                analisisOperativos /
                snapshots.length
    ...
    let estado: SaludOneDriveMensual;
...
export async function getEquipoAgentDashboard(
    req: Request,
    res: Response
) {
    try {
        const current =
            getCurrentChileYearMonth();
 ...
 */
        const horasSinConexionEnv = Number(
            process.env.AGENT_OFFLINE_AFTER_HOURS
        );

        const horasSinConexion =
            Number.isFinite(horasSinConexionEnv) &&
        ...
        const itemsBase =
            equipos.map((equipo) => {
                ...
                const estadoAgenteCalculado =
                    calcularEstadoAgente(
                        equipo.lastSeenAt,
                        horasSinConexion
                    );

                const estadoOneDriveCalculado =
                    calcularEstadoActualOneDrive(
                        equipo.detalle
                    );

                const saludMensualOneDrive =
                    calcularSaludMensualOneDrive(
                        equipo.agenteEventos
                    );

                const ultimoEventoMes =
                    equipo.agenteEventos[0] ??
                    null;

                const diasAnalizados =
                    Array.from(
                        new Set(
                            equipo.agenteEventos.map(
                                (evento) =>
                                    formatChileDateKey(
                                        evento.createdAt
                                    )
                ...
                if (
                    ...
                    alertas.push(
                        "El agente no reporta dentro del tiempo esperado."
                    );
                }
                ...
                if (
                    ...
                    alertas.push(
                        "El equipo no registra agente de inventario."
                    );
                }

                if (versionDesactualizada) {
                    alertas.push(
                        "La versión del agente no coincide con la recomendada."
                    );
                }
 ...
 */
                if (
                    ...
                    alertas.push(
                        "OneDrive está instalado, pero no se encuentra en ejecución."
                    );
                }
                ...
                if (
                    ...
                    alertas.push(
                        "OneDrive está instalado, pero no tiene un usuario detectado."
                    );
                }
                ...
                if (
                    ...
                    alertas.push(
                        "OneDrive está instalado, pero reporta advertencias."
                    );
                }
                ...
                if (
                    ...
                    alertas.push(
                        "OneDrive no se encuentra instalado en el equipo."
                    );
                }
                 ...
                 */
                if (
                    saludMensualOneDrive.estado ===
                    "INTERMITENTE"
                ) {
                    alertas.push(
                        `OneDrive tuvo una operatividad intermitente durante el mes (${saludMensualOneDrive.porcentajeOperativo}%).`
                    );
                }
                ...
                if (
                    ...
                    alertas.push(
                        `OneDrive presentó fallas durante el mes (${saludMensualOneDrive.porcentajeOperativo}% operativo).`
                    );
                }
                ...
                if (
                    ...
                    alertas.push(
                        "La clasificación del solicitante requiere revisión."
                    );
                }
                ...
                return {
                    ...
                    solicitante:
                        equipo.solicitante
                            ? {
                                id:
                                    equipo.solicitante.id_solicitante,
                                nombre:
                    ...
                    analisisMes: {
                        ...
                        diasAnalizados:
                            diasAnalizados.length,

                        ultimoAnalisis:
                            ultimoEventoMes?.createdAt ??
                            null,

                        ultimoTipo:
                            ultimoEventoMes?.tipo ??
                            null,
                    },
...
========================================================= */
         ...
         * - período mensual
         */
        const coberturaEmpresaMap =
            new Map<string, CoberturaEmpresaDashboard>();

        for (const item of itemsBase) {
            const empresaId =
                item.empresa?.id ?? null;
            ...
            coberturaEmpresaMap.set(
                key,
                current
            );
        }

        const coberturaPorEmpresa:
            CoberturaEmpresaDashboard[] =
            Array.from(
                coberturaEmpresaMap.values()
            )
                .map((empresa) => {
                    const pendientes =
                        empresa.total -
 ...
 */
        ...
        for (const item of items) {
            ...
            if (!equipoOriginal) continue;

            for (
                const evento of
                equipoOriginal.agenteEventos
            ) {
                const fecha =
                    formatChileDateKey(
                        evento.createdAt
                ...
                if (
                    oneDriveOperativo === false
                ) {
                    currentDay
                        .oneDriveConFalla += 1;
                }

                eventosPorDiaMap.set(
                    fecha,
                    currentDay
                );
            }
            ...
            Array.from(
                ...
                .map(([fecha, value]) => ({
                    ...
                    equiposAnalizados:
                        value.equipos.size,
                    oneDriveOperativos:
                        value.oneDriveOperativos,
                    oneDriveConFalla:
                        value.oneDriveConFalla,
                }))
                .sort((a, b) =>
                    a.fecha.localeCompare(b.fecha)
                );

        const versionesAgenteMap =
            new Map<string, number>();

        const versionesOneDriveMap =
            new Map<string, number>();
        ...
        for (const item of items) {
            ...
            versionesAgenteMap.set(
                agenteVersion,
                (
                    versionesAgenteMap.get(
                        agenteVersion
                    ) ?? 0
                ) + 1
            );
            ...
            const oneDriveVersion =
                item.oneDrive.version ??
                "Sin versión";

            versionesOneDriveMap.set(
                oneDriveVersion,
                (
                    versionesOneDriveMap.get(
                        oneDriveVersion
                    ) ?? 0
                ) + 1
            );
        ...
        const versionesAgente =
            Array.from(
                versionesAgenteMap.entries()
            )
        ...
        const versionesOneDrive =
            Array.from(
                versionesOneDriveMap.entries()
            )
                .map(([version, cantidad]) => ({
                    version,
                    cantidad,
        ...
        console.error(
            "❌ Error obteniendo dashboard del agente:",
            error
        );
        ...
        return res.status(500).json({
            ok: false,
            error:
                "Error interno obteniendo dashboard del agente",
        });
    }
}