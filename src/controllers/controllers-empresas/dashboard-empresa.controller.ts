...
function buildMonthlyRangeSantiago(mes: string, ano: string) {
    const desde = DateTime.fromObject(
        { year: Number(ano), month: Number(mes), day: 1, hour: 0, minute: 0, second: 0, millisecond: 0 },
...
function buildPrevMonthRange(mes: string, ano: string) {
    const dt = DateTime.fromObject(
        { year: Number(ano), month: Number(mes), day: 1 },
        { zone: "America/Santiago" }
    ).minus({ months: 1 });
...
function labelMes(dt: DateTime): string {
    // "2025-03"
...
function calcularMinutosTicket(
    createdAt: Date,
...
export async function getEmpresaDashboard(req: Request, res: Response) {
    try {
        ...
        const user = req.user as { id?: number; rol?: string; empresaId?: number | null };
        ...
        const inicioTendencia = DateTime.fromObject(
            { year: Number(ano), month: Number(mes), day: 1 },
            { zone: "America/Santiago" }
        ).minus({ months: 5 }).toUTC().toJSDate();
        ...
        const { totalMinutos: minutosVisitas, diasConVisitas: diasConVisitasMes } =
        ...
        const minutosTickets = ticketsMes.reduce(
            (acc, t) => acc + calcularMinutosTicket(t.createdAt, t.resolvedAt, t.closedAt),
            0
        );
        const ticketsMesTotal = ticketsMes.length;
        const ticketsMesResueltos = ticketsMes.filter(
        ...
        const { totalMinutos: minutosVisitasAnt } =
            calcularMinutosJornadaVisitas(visitasMesAnt);
        ...
        const minutosTicketsAnt = ticketsMesAnt.reduce(
            (acc, t) => acc + calcularMinutosTicket(t.createdAt, t.resolvedAt, t.closedAt),
            0
        );
        const horasSoporteAntConTickets = Math.round(
            (minutosVisitasAnt + minutosRemotosAnt + minutosTicketsAnt) / 60
        );
        ...
        const tendenciaSlots = new Map<
            string,
            { label: string; visitas: number; remotas: number; tickets: number; minutos: number }
        >();

        // Generar los 6 labels ordenados
        const baseMonth = DateTime.fromObject(
            { year: Number(ano), month: Number(mes), day: 1 },
            { zone: "America/Santiago" }
        );
        for (let i = 5; i >= 0; i--) {
        ...
        for (const m of remotasTendencia) {
            ...
            slot.remotas += 1;
            const d = Number(m.duracionMinutos ?? 0);
            if (Number.isFinite(d) && d > 0) slot.minutos += d;
        }
