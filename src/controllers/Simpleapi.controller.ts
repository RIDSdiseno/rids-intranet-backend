...
async function getRutEmpresaCliente(req: Request) {
...
function getDetalleVentasFromData(data: any): any[] {
  ...
  if (Array.isArray(data?.data?.detalle)) return data.data.detalle;
  if (Array.isArray(data?.data?.documentos)) return data.data.documentos;

  // Estructuras anidadas en response
  if (Array.isArray(data?.response?.ventas)) return data.response.ventas;
  if (Array.isArray(data?.response?.detalleVentas)) return data.response.detalleVentas;
  if (Array.isArray(data?.response?.detalle)) return data.response.detalle;
  if (Array.isArray(data?.response?.documentos)) return data.response.documentos;

  // Estructuras anidadas en resultado
  if (Array.isArray(data?.resultado?.ventas)) return data.resultado.ventas;
  if (Array.isArray(data?.resultado?.detalleVentas)) return data.resultado.detalleVentas;
  if (Array.isArray(data?.resultado?.detalle)) return data.resultado.detalle;
  if (Array.isArray(data?.resultado?.documentos)) return data.resultado.documentos;

  // Estructura raw, por si guardaste la respuesta original de SimpleAPI
  if (Array.isArray(data?.raw?.ventas)) return data.raw.ventas;
  if (Array.isArray(data?.raw?.detalleVentas)) return data.raw.detalleVentas;
  if (Array.isArray(data?.raw?.detalle)) return data.raw.detalle;
  if (Array.isArray(data?.raw?.documentos)) return data.raw.documentos;

  if (Array.isArray(data?.raw?.data?.ventas)) return data.raw.data.ventas;
  if (Array.isArray(data?.raw?.data?.detalleVentas)) return data.raw.data.detalleVentas;
  if (Array.isArray(data?.raw?.data?.detalle)) return data.raw.data.detalle;
  if (Array.isArray(data?.raw?.data?.documentos)) return data.raw.data.documentos;

  return [];
}

function getRutReceptorFromDoc(doc: any): string {
  return normalizeRut(
    doc?.rutReceptor ??
    doc?.RutReceptor ??
    doc?.RUTReceptor ??
...
function getNombreReceptorFromDoc(doc: any): string {
  return String(
...
function filtrarVentasPorRutReceptor(data: any, rutCliente: string) {
...
function sumarMontosDetalleVentas(detalleVentas: any[]) {
  return detalleVentas.reduce(
    (acc, doc) => {
      acc.totalDocumentos += 1;

      acc.montoExento += Number(
        doc?.montoExento ?? doc?.MntExe ?? doc?.monto_exento ?? 0
      );

      acc.montoNeto += Number(
        doc?.montoNeto ?? doc?.MntNeto ?? doc?.monto_neto ?? 0
      );

      acc.montoIVA += Number(
        doc?.montoIVA ?? doc?.IVA ?? doc?.monto_iva ?? 0
      );

      acc.montoTotal += Number(
        doc?.montoTotal ?? doc?.MntTotal ?? doc?.monto_total ?? 0
      );
...
function resolverRutEmpresa(empresaRaw: unknown): EmpresaRutResult {
  ...
  try {
    ...
    if (user?.rol === "CLIENTE") {
      ...
      const resultados = await Promise.all(
        EMPRESAS_EMISORAS_CLIENTE.map(async (emisor) => {
          ...
          const dataFiltrada = filtrarVentasPorRutReceptor(
            resultado.data,
            rutCliente
          );
      ...
      const detalleVentas = resultados.flatMap((r) =>
        getDetalleVentasFromData(r.data)
      );
