...
// =====================================================
type PlantillaMasivaItemInput = {
    productoId: number;
};

type PlantillaMasivaEntidadInput = {
    entidadId: number;
    cantidades: Array<{
        productoId: number;
        cantidad: number;
...
export async function listarPlantillasMasivas(req: Request, res: Response) {
    try {
...
export async function crearPlantillaMasiva(req: Request, res: Response) {
    try {
        const user = (req as any).user;
        ...
        const {
            ...
            nombre: string;
            descripcion?: string | null;
            productos: PlantillaMasivaItemInput[];
            entidades: PlantillaMasivaEntidadInput[];
        };
...
export async function generarCotizacionesDesdePlantilla(
    req: Request,
    res: Response
) {
    try {
        ...
        const { nombreEjecucion } = req.body as {
            nombreEjecucion?: string | null;
        };
        ...
        const resultados = await prisma.$transaction(async (tx) => {
            ...
            for (const entidadPlantilla of plantilla.entidades) {
                const itemsValidos = entidadPlantilla.cantidades.filter(
                    (cantidad) => Number(cantidad.cantidad) > 0
                );
                ...
                const itemsNormalizados: ItemNormalizadoMasivo[] = itemsValidos.map((item) =>
                    calcularItemMasivo(item.producto, Number(item.cantidad))
                );
...
export async function actualizarPlantillaMasiva(req: Request, res: Response) {
    try {
        const plantillaId = Number(req.params.id);
        ...
        const plantillaExistente =
            await prisma.cotizacionMasivaPlantilla.findUnique({
                where: {
                    id: plantillaId,
                },
                select: {
        ...
        await prisma.$transaction(
            async (tx) => {
                const entidadesActuales =
                    await tx.cotizacionMasivaPlantillaEntidad.findMany({
                        where: {
                            plantillaId,
                        },
                        select: {
...
export async function desactivarPlantillaMasiva(req: Request, res: Response) {
    try {
        const plantillaId = Number(req.params.id);
