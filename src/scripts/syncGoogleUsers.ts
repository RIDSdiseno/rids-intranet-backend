 ...
 */
function parseDomainMap(): Record<string, number> {
  ...
  return raw.split(",").reduce<Record<string, number>>((acc, pair) => {
    const [d, idStr] = pair.split(":").map((s) => s.trim());
    const id = Number(idStr);
    if (d && Number.isFinite(id)) acc[d] = id;
    return acc;
  }, {});
}

const DOMAIN_TO_EMPRESA: Record<string, number> = parseDomainMap();

/* =========================================
   Tipos y helpers
   ========================================= */
type MaybeFlags = { created?: boolean; updated?: boolean; skipped?: boolean };
...
/* =========================================
   Core
   ========================================= */
async function syncDomain(domain: string, empresaId: number) {
  console.log(
    `[${nowISO()}][sync] dominio=${domain} → empresaId=${empresaId} (activeOnly=${ONLY_ACTIVE}, dryRun=${DRY_RUN}, conc=${CONCURRENCY})`
  );

  let users: GoogleDirectoryUser[] = [];
  try {
    users = (await listAllUsers(domain)) as GoogleDirectoryUser[];
  } catch (e: any) {
  ...
  const clean = users.filter((u) => {
    ...
    if (ONLY_ACTIVE && !isActive(u)) return false;
    return true;
  });
  ...
  if (clean.length === 0) {
    console.log(
      `[sync] ${domain}: no hay usuarios a procesar (recibidos=${counters.recibidos}, omitidos=${counters.omitidos}).`
    );
    return counters;
  }
  ...
  for (const batch of batches) {
    ...
    const results = await Promise.allSettled(
      batch.map(async (raw) => {
        ...
        if (DRY_RUN) {
          ...
          return { email, flags: { skipped: true, dryRun: true } as MaybeFlags };
        }

        const res: MaybeFlags | any = await upsertSolicitanteFromGoogle_min(user, empresaId);
        return { email, flags: res as MaybeFlags };
      })
    );

    for (const r of results) {
      if (r.status === "fulfilled") {
        const { flags } = r.value as { email: string; flags?: MaybeFlags };
        if (flags && typeof flags === "object") {
          if (flags.created) counters.creados++, counters.ok++;
          else if (flags.updated) counters.actualizados++, counters.ok++;
        ...
        const who =
          (reason && reason.user && reason.user.primaryEmail) ||
          reason?.email ||
          "(email no disponible)";
    ...
    // Telemetría por lote
    console.log(
      `[sync] ${domain}: batch ${batchIndex}/${batches.length} → ok=${counters.ok}, creados=${counters.creados}, actualizados=${counters.actualizados}, fallidos=${counters.fallidos}`
    );
  }

  console.log(
    `[sync] ${domain}: recibidos=${counters.recibidos} ` +
...
async function main() {
  ...
  if (pairs.length === 0) {
    console.error(
      '[sync] ERROR: No hay dominios configurados. Usa SYNC_DOMAIN_MAP="dominio1:empresaId1,..."'
    );
    process.exit(2);
  }
  ...
  const perDomain: Record<string, Counters> = {};
  ...
  // Resumen global
  const summary = Object.values(perDomain).reduce(
    (acc, c) => {
