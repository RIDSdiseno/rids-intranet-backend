...
async function obtenerEquipoIdsPorActividad(params) {
    const { auditTecnicoId, auditFrom, auditTo, auditAction, } = params;
    /* ====================================================
       1. AuditLog
    ==================================================== */
    const logs = await prisma.auditLog.findMany({
        where: {
            actorId: auditTecnicoId,
            entity: {
                in: [
                    "Equipo",
                    "DetalleEquipo",
     ...
     */
    const tiposEventoAgente = auditAction ===
        "CREATE"
        ? [
            "INVENTORY_CREATED",
        ]
    ...
    const equipoIdsDesdeAgente = eventosAgente
...
export async function getInventarioByEmpresa(params) {
    const { empresaId, empresaNombre, marca, estado, propiedad, propietarioExterno, anioPcDesde, anioPcHasta, anioPcOrigen, createdFrom, createdTo, updatedFrom, updatedTo, agente, agenteDesde, agenteHasta, auditTecnicoId, auditFrom, auditTo, auditAction, solicitanteMultiplesEquipos, } = params;
    /* ====================================================
       WHERE dinámico
    ==================================================== */
    const AND = [
    ...
    /* ====================================================
       Propietario externo
    ==================================================== */
    if (propietarioExterno) {
        AND.push({
            propietarioExterno: {
                contains: propietarioExterno,
                mode: "insensitive",
            },
        });
    }
    ...
    ==================================================== */
    if (auditTecnicoId) {
        const equipoIdsActividad = await obtenerEquipoIdsPorActividad({
            auditTecnicoId,
            /*
  ...
  ==================================================== */
    if (solicitanteMultiplesEquipos ===
        "MULTIPLES") {
        /*
        ...
        AND.push({
            idSolicitante: {
                in: solicitanteIdsMultiplesEquipos,
            },
        });
    }
    /* ====================================================
