...
const TRANSICIONES_ESTADO_AGENDA = {
...
function validarTransicionEstadoAgenda(estadoActual, estadoSolicitado, visitaStatus) {
    if (visitaStatus === EstadoVisita.COMPLETADA && estadoSolicitado !== EstadoAgenda.COMPLETADA) {
        throw new AgendaStateTransitionError("Una agenda con formulario de visita completado no puede volver a un estado anterior.");
    }
    if (!TRANSICIONES_ESTADO_AGENDA[estadoActual].has(estadoSolicitado)) {
        throw new AgendaStateTransitionError(`No se permite cambiar la agenda de ${estadoActual} a ${estadoSolicitado}.`);
    }
}
/* ======================================================
 ...
 */
export function normalizarFechaDesdeString(fecha) {
...
async function adjuntarFormularioVisita(visitas) {
    ...
    return visitas.map((visita) => {
        ...
        return serializarAgendaVisita({
            ...
            inconsistenciaEstado: formulario?.status === EstadoVisita.COMPLETADA &&
                "estado" in visita &&
...
async function buscarEmpresaPorCoincidenciaParcial(nombreNormalizado) {
...
async function resolverEmpresaDesdeOutlook(nombre) {
...
function getNombreEmpresaAgenda(visita) {
    return (visita.empresa?.nombre?.trim() ||
...
function buildAgendaOutlookLocation(visita) {
...
function buildAgendaDateTime(fecha, hora) {
    const horaNormalizada = hora?.trim();
    if (!horaNormalizada)
        return undefined;
    return `${formatearFechaAgenda(fecha)}T${horaNormalizada}:00`;
}
function buildAgendaOutlookSubject(visita) {
    const nombreTecnico = visita.tecnicos?.[0]?.tecnico?.nombre?.trim() || "Técnico";
    const nombreEmpresa = getNombreEmpresaAgenda(visita);
    return `${nombreTecnico} - ${nombreEmpresa}`;
}
function buildAgendaOutlookBody(visita) {
    const nombreEmpresa = getNombreEmpresaAgenda(visita);
...
function buildAgendaOutlookAttendees(visita) {
    const seen = new Set();
    const attendees = [];
...
function buildAgendaOutlookCategory(visita) {
    const tecnicoNombre = visita.tecnicos?.[0]?.tecnico?.nombre?.trim().toLowerCase() || "";
    if (tecnicoNombre.includes("manuel ahumada"))
        return "Manuel Ahumada";
    if (tecnicoNombre.includes("georges martinez"))
        return "Georges Martinez";
    if (tecnicoNombre.includes("rudy calsin"))
        return "Rudy Calsin";
    if (tecnicoNombre.includes("constanza"))
        return "Constanza";
    if (tecnicoNombre.includes("diego"))
        return "Diego";
    if (tecnicoNombre.includes("gonzalo"))
        return "Gonzalo";
    if (tecnicoNombre.includes("ignacio"))
        return "Ignacio";
    if (tecnicoNombre.includes("sebastian"))
        return "Sebastian";
    return "OFICINA";
}
/* ======================================================
 ...
 */
async function getTecnicosUltimoSabado() {
 ...
 */
async function elegirTecnicosSabado(todosTecnicos) {
    const excluidos = await getTecnicosUltimoSabado();
    let candidatos = todosTecnicos.filter((t) => !excluidos.includes(t.id_tecnico));
 ...
 */
export async function generarMallaMensual(year, month, empresaIds, includeOficina) {
    ...
    const dias = Array.from({ length: totalDias }, (_, d) => new Date(Date.UTC(primerDia.getUTCFullYear(), primerDia.getUTCMonth(), primerDia.getUTCDate() + d)));
    /* =====================
    ...
    const existeSet = new Set(visitasExistentes.map((v) => {
    ...
    for (const fecha of dias) {
        ...
        if (esLMV && N > 0) {
            ...
            for (let i = 0; i < tecnicosLMV.length; i++) {
                ...
                if (!asignaciones.has(e.id_empresa))
                    asignaciones.set(e.id_empresa, []);
        ...
        if (includeOficina && poolDelDia.length > 0) {
            ...
            for (const [empresaId, tecnicoIds] of asignaciones) {
                ...
                if (filtrados.length === 0)
                    asignaciones.delete(empresaId);
                else
                    asignaciones.set(empresaId, filtrados);
            }
        }
        ...
        const tecnicosEnEmpresas = new Set();
        for (const ids of asignaciones.values())
            ids.forEach(id => tecnicosEnEmpresas.add(id));
    ...
    ===================== */
    const claveNuevas = new Set(nuevas.map((v) => `${v.fecha.toISOString().slice(0, 10)}|${v.empresaId ?? "null"}|${v.tipo}`));
 ...
 */
export async function getAgendaMensual(year, month, filtros) {
...
function parseAgendaOutlookSubject(subject) {
...
function parseAgendaOutlookDateTime(value) {
...
function buildAgendaOutlookMonthRange(year, month) {
    const primerDia = new Date(Date.UTC(year, month - 1, 1));
    const ultimoDia = new Date(Date.UTC(year, month, 0));
    const inicio = getLunesDeLaSemana(primerDia);
    const fin = getDomingoDeLaSemana(ultimoDia);
...
async function buscarTecnicoPorNombreOutlook(nombre) {
    ...
    const porIgual = todos.find((t) => normalizarTextoNombre(t.nombre) === nombreNormalizado);
    if (porIgual)
        return porIgual;
    ...
    if (primerToken && primerToken.length >= 3) {
        const porPrimerToken = todos.find((t) => normalizarTextoNombre(t.nombre).startsWith(primerToken));
        if (porPrimerToken)
            return porPrimerToken;
    }
    return null;
}
async function resolverTecnicoDesdeOutlook(params) {
    ...
    const candidatos = [params.categories?.[0]?.trim() || null, tecnicoDesdeSubject]
        .filter((nombre, index, arr) => Boolean(nombre) && arr.indexOf(nombre) === index);
...
async function resolverTecnicosDesdeOutlook(params) {
    ...
    const attendeeEmails = Array.from(new Set((params.attendees ?? [])
        .map((attendee) => attendee?.emailAddress?.address?.trim().toLowerCase())
        .filter((email) => Boolean(email) && email !== supportEmail)));
    if (attendeeEmails.length > 0) {
        ...
        const tecnicoPorEmail = new Map(tecnicosPorEmail
        ...
        for (const email of attendeeEmails) {
            ...
            if (!tecnico || seen.has(tecnico.id_tecnico))
                continue;
...
async function sincronizarTecnicosAgendaOutlook(agendaId, tecnicoIds) {
...
async function buscarAgendaCoincidenteDesdeOutlook(params) {
    ...
    if (tecnicoIdsObjetivo.length === 0)
        return null;
    ...
    const esOficina = params.empresaId === null &&
    ...
    for (const candidata of candidatas) {
        const tecnicoIdsCandidata = Array.from(new Set(candidata.tecnicos.map((tecnico) => tecnico.tecnicoId)));
        const cantidadCoincidencias = tecnicoIdsCandidata.filter((tecnicoId) => tecnicoIdsObjetivoSet.has(tecnicoId)).length;
        if (cantidadCoincidencias === 0)
            continue;
        const esCoincidenciaExacta = cantidadCoincidencias === tecnicoIdsObjetivoSet.size &&
...
export async function sincronizarAgendaDesdeOutlook(year, month) {
    ...
    const visitasSegurasParaEliminar = visitasEliminadasEnOutlook.filter((visita) => !visita.visita &&
    ...
    for (const event of events) {
        try {
            ...
            const { empresaId, empresaExternaNombre, } = await resolverEmpresaDesdeOutlook(empresaDesdeSubject);
 ...
 */
export async function getAgendaPorDia(fecha) {
 ...
 */
export async function actualizarAgendaVisita(id, datos) {
    const { fecha: fechaStr, estado, notas, mensaje, horaInicio, horaFin, empresaId, sucursalId, } = datos;
 ...
 */
export async function reasignarTecnicos(agendaId, nuevosTecnicoIds) {
    ...
    if (visitaActualizada?.outlookEventId &&
 ...
 */
export async function eliminarAgendaVisita(id) {
    const visita = await prisma.agendaVisita.findUnique({
        where: { id },
        select: {
            outlookEventId: true,
 ...
 */
export async function eliminarAgendaVisitasEnLote(ids) {
    const eliminadas = [];
    const errores = [];
    for (const id of ids) {
        try {
 ...
 */
export async function eliminarMallaMensual(year, month) {
    ...
    if (agendasEliminables.length === 0)
        return { eliminadas: 0 };
...
export async function enviarRecordatoriosPendientes() {
    ...
    for (const visita of candidatas) {
        ...
        if (diferencia < 55 || diferencia > 60)
            continue;
        const nombreEmpresa = getNombreEmpresaAgenda(visita);
        const fechaStr = visita.fecha.toISOString().slice(0, 10);
        const tecnicosHtml = visita.tecnicos
