...
function seleccionarCachesMensuales(caches, tipoPreferido, tipoRcv) {
    ...
    for (const cache of caches) {
        const mes = String(cache.mes ??
            "")
            .padStart(2, "0");
        if (!mes) {
            continue;
        }
        const existente = cachePorMes.get(mes);
        if (!existente) {
            cachePorMes.set(mes, cache);
            continue;
        }
        const prioridadActual = cache.tipo ===
            tipoPreferido
            ? 2
            : 1;
        const prioridadExistente = existente.tipo ===
            tipoPreferido
            ? 2
            : 1;
        if (prioridadActual >
    ...
    return Array.from(cachePorMes.values())
        .filter((cache) => getDocumentosRcv(cache.data, tipoRcv).length >
        0)
        .sort((a, b) => Number(a.mes) -
        Number(b.mes));
}
const MESES = [
    "Enero",
...
function getDocumentosRcv(data, tipo) {
...
function getMontoTotal(doc) {
    return toNumber(doc?.["Monto total"] ??
        doc?.["Monto Total"] ??
        doc?.montoTotal ??
        0);
}
function getMontoNeto(doc) {
    return toNumber(doc?.["Monto Neto"] ??
        doc?.montoNeto ??
        0);
}
/* =========================================================
...
export async function obtenerDashboardFinanzas(params) {
     ...
     * =====================================================
     */
    const caches = await prisma.siiApiCache.findMany({
        where: {
            empresaKey,
            tipo: {
                in: [
    ...
    const cachesCompras = await prisma.siiApiCache.findMany({
        where: {
            empresaKey,
            tipo: {
                in: [
    ...
    const cachesComprasMensuales = seleccionarCachesMensuales(cachesCompras, "baseapi-rcv-compras", "compras");
    /*
 * =====================================================
 * Normalizar cache:
 ...
 */
    ...
    const cachesMensuales = Array.from(cachePorMes.values()).sort((a, b) => Number(a.mes) -
        Number(b.mes));
    const cachesMensualesConDatos = cachesMensuales.filter((cache) => getDocumentosRcv(cache.data, "ventas").length >
        0);
    /*
     * =====================================================
     ...
     * =====================================================
     */
    const desde = new Date(`${ano}-01-01T00:00:00.000Z`);
    const hasta = new Date(`${ano + 1}-01-01T00:00:00.000Z`);
     ...
     * =====================================================
     */
    const recordatorios = await prisma.rcvRecordatorioEnvio.findMany({
        where: {
            empresaKey,
            tipoRcv: "ventas",
            estado: "ENVIADO",
     ...
     * =====================================================
     */
    for (const cache of cachesMensualesConDatos) {
        const mesNumero = Number(cache.mes);
        if (!Number.isFinite(mesNumero) ||
            mesNumero <
                1 ||
            mesNumero >
                12) {
        ...
        const documentos = getDocumentosRcv(cache.data, "ventas");
        ...
        const documentosConOrigen = documentos.map((doc) => ({
            ...doc,
            empresaOrigen: empresaKey,
        }));
        const documentosAnotados = await anotarDocumentosCobranza(documentosConOrigen, "ventas", empresaKey);
        const bucket = meses[mesNumero -
            1];
        if (!bucket) {
            continue;
        }
        for (const doc of documentosAnotados) {
            const total = getMontoTotal(doc);
            const neto = getMontoNeto(doc);
 ...
 * =====================================================
 */
    for (const cache of cachesComprasMensuales) {
        const mesNumero = Number(cache.mes);
        if (!Number.isFinite(mesNumero) ||
            mesNumero < 1 ||
            mesNumero > 12) {
        ...
        const documentos = getDocumentosRcv(cache.data, "compras");
        if (documentos.length ===
            0) {
            continue;
        }
    ...
    for (const conciliacion of conciliaciones) {
        ...
        const month = conciliacion
            .conciliadoAt
            .getUTCMonth();
        const bucket = meses[month];
        if (!bucket) {
            continue;
        }
        bucket.pagado +=
    ...
    resumen.totalDocumentos =
        cachesMensualesConDatos.reduce((total, cache) => total +
            getDocumentosRcv(cache.data, "ventas").length, 0);
    resumen.totalDocumentosCompras =
        cachesComprasMensuales.reduce((total, cache) => total +
            getDocumentosRcv(cache.data, "compras").length, 0);
    const ultimaActualizacion = [
        ...cachesMensualesConDatos,
        ...cachesComprasMensuales,
    ]
        .map((cache) => cache.updatedAt)
        .sort((a, b) => b.getTime() -
        a.getTime())[0] ??
        null;
    return {
        ...
        meses,
        ultimaActualizacion,
        mesesConCache: cachesMensualesConDatos
            .map((item) => item.mes)
            .filter(Boolean),
        mesesConCompras: cachesComprasMensuales
            .map((item) => item.mes)
            .filter(Boolean),
        cacheUtilizado: cachesMensualesConDatos.map((item) => ({
            mes: item.mes,
            tipo: item.tipo,
