...
function getXmlBase64FromBaseApiResponse(data) {
    return (data?.data?.documento?.xml_base64 ??
        data?.documento?.xml_base64 ??
        null);
}
function getDocumentoFromBaseApiResponse(data) {
    return data?.data?.documento ?? data?.documento ?? null;
}
function findFirstXmlElement(parent, tagName) {
    const all = Array.from(parent.getElementsByTagName("*"));
    return all.find((el) => el.localName === tagName) ?? null;
}
function getTextFromXml(parent, tagName) {
    if (!parent)
        return "";
    const all = Array.from(parent.getElementsByTagName("*"));
...
function extractTimbreFrmtFromXml(xmlRaw) {
    try {
        const parser = new DOMParser();
        const xml = parser.parseFromString(xmlRaw, "text/xml");
        ...
        if (!ted)
            return null;
        const frmtEl = Array.from(ted.getElementsByTagName("*")).find((el) => el.localName === "FRMT");
...
function mapFacturaCacheToBaseApiLikeResponse(factura) {
    return {
        success: true,
        data: {
            documento: {
...
export async function consultarDtePorFolioBaseApi(params) {
    const { empresa, periodo, folio, tipoDTE = 33, forceRefresh = false, } = params;
        ...
        const factura = await prisma.$transaction(async (tx) => {
            const upserted = await tx.facturaDTE.upsert({
                ...
                create: {
                    ...
                    folio: folioFinal,
                    tipoDTE: tipoDTEFinal,
                    tipoDTEString: documentoBaseApi?.tipo_dte_nombre ??
...
//# sourceMappingURL=baseapi-dte.service.js.map