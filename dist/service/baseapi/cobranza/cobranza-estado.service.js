...
function obtenerFechaEmisionDocumento(doc) {
    const candidates = [
        doc?.["Fecha Docto"],
        doc?.["Fecha Documento"],
        doc?.["Fecha Emisión"],
...
function sumarDiasFecha(fecha, dias) {
    const result = normalizarFechaDia(fecha);
...
export async function anotarDocumentoCobranza(doc, tipoRcv, empresaFallback) {
    const resultados = await anotarDocumentosCobranza([doc], tipoRcv, empresaFallback);
    return (resultados[0] ??
        doc);
}
...
function getCreditoRutKey(rut) {
    return normalizarRut(rut);
}
function getFacturaDteKey(empresaKey, tipoDoc, folio) {
    return [
        String(empresaKey ??
            "")
            .trim()
...
function getRecordatorioKey(empresaKey, tipoRcv, tipoDoc, folio) {
    return [
        String(empresaKey ??
            "")
            .trim()
...
function crearResumenAutomatizacionVacio() {
    return {
        ...
        errores: 0,
        ultimoEnvioAt: null,
        ultimoRegistroAt: null,
        ultimoTipoRecordatorio: null,
        ultimoCicloVencimiento: null,
        destinatarios: [],
        historial: [],
    };
}
function resumirAutomatizacionCobranza(recordatorios) {
     ...
     */
    const ordenados = recordatorios
        .slice()
        .sort((a, b) => b.createdAt.getTime() -
        a.createdAt.getTime());
    const enviados = ordenados.filter((item) => item.estado ===
        "ENVIADO" ||
        Boolean(item.enviadoAt));
    const pendientes = ordenados.filter((item) => item.estado ===
        "PENDIENTE");
    const procesando = ordenados.filter((item) => item.estado ===
        "PROCESANDO");
    const errores = ordenados.filter((item) => item.estado ===
        "ERROR");
    /*
    ...
    const ultimoRegistro = ordenados[0] ??
        null;
    const ultimoTipoRecordatorio = ultimoRegistro
        ?.tipoRecordatorio ??
        null;
    const ultimoCicloVencimiento = ultimoRegistro
        ?.cicloVencimiento ??
        null;
    /*
     ...
     */
    const ultimoCiclo = ultimoRegistro
        ? ordenados.filter((item) => item.tipoRecordatorio ===
            ultimoRegistro.tipoRecordatorio &&
            item.cicloVencimiento ===
                ultimoRegistro.cicloVencimiento)
        : [];
    const enviadosUltimoCiclo = ultimoCiclo.filter((item) => item.estado ===
        "ENVIADO" ||
        Boolean(item.enviadoAt));
    const pendientesUltimoCiclo = ultimoCiclo.filter((item) => item.estado ===
        "PENDIENTE");
    const procesandoUltimoCiclo = ultimoCiclo.filter((item) => item.estado ===
        "PROCESANDO");
    const erroresUltimoCiclo = ultimoCiclo.filter((item) => item.estado ===
        "ERROR");
    let estado = "ENVIADO";
    if (erroresUltimoCiclo.length >
        0 &&
    ...
    else if (erroresUltimoCiclo.length >
        0) {
        estado =
            "ERROR";
    }
    else if (procesandoUltimoCiclo.length >
        0) {
        estado =
            "PROCESANDO";
    }
    else if (pendientesUltimoCiclo.length >
        0) {
        estado =
            "PENDIENTE";
    }
    const ultimoEnvio = enviados
        .filter((item) => item.enviadoAt)
        .sort((a, b) => (b.enviadoAt
        ?.getTime() ??
        0) -
        (a.enviadoAt
            ?.getTime() ??
    ...
    return {
        ...
        ultimoEnvioAt: ultimoEnvio
            ?.enviadoAt ??
            null,
        ultimoRegistroAt: ultimoRegistro
            ?.createdAt ??
            null,
         ...
         */
        destinatarios: ultimoCiclo.map((item) => ({
            email: item.emailDestino,
            nombre: item.nombreDestino,
            estado: item.estado,
         ...
         */
        historial: ordenados.map((item) => ({
            id: item.id,
            tipoRecordatorio: item.tipoRecordatorio,
            cicloVencimiento: item.cicloVencimiento,
            email: item.emailDestino,
            nombre: item.nombreDestino,
            estado: item.estado,
            enviadoAt: item.enviadoAt,
            error: item.error,
            intentos: item.intentos,
            ultimoIntentoAt: item.ultimoIntentoAt,
...
export async function obtenerEstadosDocumentosCobranza(documentos, tipoRcv, empresaFallback) {
    ...
    const tiposDteNumericos = Array.from(new Set(tiposDoc
        .map((value) => Number(value))
        .filter(Number.isFinite)));
    const foliosNumericos = Array.from(new Set(folios
        .map((value) => Number(value))
        .filter(Number.isFinite)));
    const [conciliaciones, vencimientos, recordatorios, facturasDte, receptoresCobranza, detallesEmpresa,] = await Promise.all([
    ...
    for (const detalle of detallesEmpresa) {
        ...
        const rut = getCreditoRutKey(detalle.rut);
        if (!rut) {
            continue;
        }
        diasCreditoPorRut.set(rut, {
            dias: detalle.diasCredito,
            origen: "DETALLE_EMPRESA",
        });
    }
    /*
    ...
    for (const receptor of receptoresCobranza) {
        ...
        const rut = getCreditoRutKey(receptor.rut);
        if (!rut) {
            continue;
        }
        diasCreditoPorRut.set(rut, {
            dias: receptor.diasCredito,
            origen: "RECEPTOR_COBRANZA",
        });
    }
    for (const factura of facturasDte) {
        if (!factura.fechaVencimiento) {
            continue;
        }
        const key = getFacturaDteKey(String(factura.empresaAlias), String(factura.tipoDTE), String(factura.folio));
        facturaDteMap.set(key, factura.fechaVencimiento);
    }
    for (const recordatorio of recordatorios) {
        const key = getRecordatorioKey(recordatorio.empresaKey, recordatorio.tipoRcv, recordatorio.tipoDoc, recordatorio.folio);
        const actuales = recordatoriosMap.get(key) ??
            [];
        actuales.push(recordatorio);
        recordatoriosMap.set(key, actuales);
    }
    ...
    for (const documento of documentos) {
        ...
        const recordatorioKey = getRecordatorioKey(empresaKey, tipoRcv, tipoDoc, folio);
        const recordatoriosDocumento = recordatoriosMap.get(recordatorioKey) ?? [];
        const automatizacion = resumirAutomatizacionCobranza(recordatoriosDocumento);
        /*
         * 1. CONCILIACIÓN
         */
 ...
 */
        const facturaDteKey = getFacturaDteKey(empresaKey, tipoDoc, folio);
        const fechaFacturaDte = facturaDteMap.get(facturaDteKey);
        if (fechaFacturaDte) {
            const fecha = normalizarFechaDia(fechaFacturaDte);
            const dias = calcularDiasDiferencia(fecha, ahora);
            resultados.push({
                documento,
                estado: {
         ...
         * 5. DÍAS DE CRÉDITO
         */
        const credito = diasCreditoPorRut.get(rutContraparte);
        if (credito &&
            credito.dias >= 0) {
            const fechaEmision = obtenerFechaEmisionDocumento(documento);
            if (fechaEmision) {
                const fechaVencimiento = sumarDiasFecha(fechaEmision, credito.dias);
                const dias = calcularDiasDiferencia(fechaVencimiento, ahora);
                /*
                ...
                resultados.push({
                    ...
                    estado: {
                        ...
                        fechaVencimientoIso: fechaVencimiento
                            .toISOString()
                            .slice(0, 10),
                        diasDiferencia: dias,
                        conciliada: false,
                        origenVencimiento: credito.origen,
                    },
                    automatizacion,
                });
                continue;
...
export async function anotarDocumentosCobranza(documentos, tipoRcv, empresaFallback) {
    ...
    return evaluados.map(({ documento, estado, automatizacion }) => {
        const result = {
            ...documento,
            estadoPago: estado.estadoPago,
            fechaVencimientoCobranza: estado.fechaVencimientoIso,
            diasDiferenciaCobranza: estado.diasDiferencia,
            diasCreditoCobranza: typeof documento
                ?.diasCreditoCobranza ===
                "number"
                ? documento
                    .diasCreditoCobranza
                : null,
            conciliada: estado.conciliada,
            origenVencimiento: estado.origenVencimiento,
            cobranzaAutomatica: automatizacion,
        };
