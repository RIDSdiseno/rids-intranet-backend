...
function getDocumentosFromCache(data) {
...
function getValue(doc, keys, fallback = null) {
...
function addToMap(map, key, initial, updater) {
 ...
 */
function calcularDeltaPct(actual, anterior) {
    if (anterior === 0)
        return null;
    return Math.round(((actual - anterior) / anterior) * 100);
}
export async function getBaseApiRcvDashboard(params) {
    ...
    const montos = documentos.reduce((acc, doc) => {
        acc.montoNeto += toNumber(getValue(doc, ["Monto Neto", "montoNeto"], 0));
        acc.montoIva += toNumber(getValue(doc, ["Monto IVA", "Monto Iva", "Monto IVA Recuperable", "montoIva", "montoIVA"], 0));
    ...
    const montoTotalAnterior = docsAnterior.reduce((sum, doc) => sum + toNumber(getValue(doc, ["Monto total", "Monto Total", "montoTotal"], 0)), 0);
    ...
    for (const doc of documentos) {
        ...
        const montoIva = toNumber(getValue(doc, ["Monto IVA", "Monto Iva", "Monto IVA Recuperable", "montoIva", "montoIVA"], 0));
        const montoTotal = toNumber(getValue(doc, ["Monto total", "Monto Total", "montoTotal"], 0));
        const fecha = parseFechaDia(getValue(doc, ["Fecha Docto", "Fecha Recepcion", "fechaDocto", "fechaEmision"]));
        contrapartes.add(rut);
        addToMap(porTipoDocumentoMap, tipoDoc, { tipoDocumento: tipoDoc, cantidad: 0, montoNeto: 0, montoIva: 0, montoTotal: 0 }, (row) => { row.cantidad += 1; row.montoNeto += montoNeto; row.montoIva += montoIva; row.montoTotal += montoTotal; });
        addToMap(contraparteMap, rut, { rut, nombre, cantidad: 0, montoNeto: 0, montoIva: 0, montoTotal: 0 }, (row) => { row.cantidad += 1; row.montoNeto += montoNeto; row.montoIva += montoIva; row.montoTotal += montoTotal; });
        addToMap(porDiaMap, fecha, { fecha, cantidad: 0, montoTotal: 0 }, (row) => { row.cantidad += 1; row.montoTotal += montoTotal; });
    }
    // ── Ordenar resultados ────────────────────────────────────────────────────
    const porTipoDocumento = Array.from(porTipoDocumentoMap.values()).sort((a, b) => b.montoTotal - a.montoTotal);
    ...
    return {
        ...
        kpis: {
            ...
            promedioDocumento: documentos.length > 0 ? Math.round(montos.montoTotal / documentos.length) : 0,
...
//# sourceMappingURL=baseapi-rcv-dashboard.service.js.map