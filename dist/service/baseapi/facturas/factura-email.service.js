...
function getNombreDocumento(tipoDoc) {
    if (tipoDoc ===
        "34") {
...
/* =========================================================
   ASUNTO
========================================================= */
export function construirAsuntoFactura(params) {
    const empresa = getNombreEmpresaCobranza(params
        .empresaKey);
    const documento = getNombreDocumento(params
        .tipoDoc);
...
/* =========================================================
   HTML
========================================================= */
function construirHtmlFactura(params) {
    const config = getConfigCorreoEmpresa(params
        .empresaKey);
    ...
    const nombre = params
        ...
        ?.trim() ||
        "Estimado(a)";
    const monto = formatearMontoClp(params
        .montoTotal);
    const fechaEmision = formatearFechaChile(params
        .fechaEmision);
    const documento = getNombreDocumento(params
        .tipoDoc);
    const datosPagoHtml = [
...
export async function enviarCorreoFactura(params) {
    try {
        ...
        const html = construirHtmlFactura(params);
        /*
 * =========================================================
 * SEGURIDAD DE ENVÍO
 * =========================================================
 ...
 */
        ...
        if (testEmail) {
            ...
            if (!envioRealHabilitado) {
                throw new Error("Envío real de facturas bloqueado: configura FACTURA_TEST_EMAIL o FACTURA_LIVE_SEND=true.");
            }
            to =
                emailDestino;
            modoPrueba =
        ...
        console.log("[FACTURA EMAIL] 📤 Preparando correo", {
            empresa: params.empresaKey,
            folio: params.folio,
            ...
            envioRealHabilitado,
            asunto,
            adjuntaPdf: Boolean(params.adjuntoPdf),
            archivoPdf: params
                .adjuntoPdf
                ?.filename ??
                null,
        ...
        const message = error instanceof Error
            ? error.message
            : String(error);
        console.error("[FACTURA EMAIL] ❌ Error enviando correo", {
            folio: params
                .folio,
            email: params
                .emailDestino,
