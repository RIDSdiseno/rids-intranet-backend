...
====================================================== */
/*
 * Archivos menores a este tamaño pueden ir
 * directamente como fileAttachment.
 *
 * Desde este tamaño utilizamos upload session.
 */
const GRAPH_SMALL_ATTACHMENT_LIMIT_BYTES = 3 * 1024 * 1024;
/*
 * Chunk para upload session.
 *
 * Utilizamos múltiplos de 320 KiB.
 *
 * 320 KiB × 10 = 3.125 MiB aprox.
 */
const GRAPH_UPLOAD_CHUNK_SIZE = 320 * 1024 * 10;
/* ======================================================
...
class GraphReaderService {
    ...
    async findEmpresaByEmail(fromEmail) {
        ...
        const empresaPorSubdominio = empresasConDominios.find((empresa) => empresa.dominios.some((d) => {
 ...
 */
        const recordatorioAt = new Date(Date.now() - 1000);
        try {
            await sincronizarRecordatorioTicketParaTodos({
                ticketId: ticket.id,
                titulo: `Nuevo ticket recibido #${ticket.id}`,
                descripcion: data.bodyText?.slice(0, 180)?.trim() ||
                    `Se recibió un nuevo ticket: ${ticket.subject}`,
                recordatorioAt,
            });
        }
        catch (error) {
            console.error(`⚠️ Error creando recordatorios globales por nuevo ticket #${ticket.id}:`, error);
        }
        /* =============================
           7️⃣ EVENTOS
        ============================= */
 ...
 */
        ...
        try {
            ...
            console.error(`⚠️ Error creando recordatorios globales por respuesta del solicitante para ticket #${ticketId}:`, error);
        }
    ...
    getGraphAttachmentSize(contentBytes) {
        return Buffer.byteLength(contentBytes, "base64");
    }
    async uploadLargeAttachmentToDraft(client, draftId, att) {
        ...
        if (!uploadUrl) {
            throw new Error(`Microsoft Graph no devolvió uploadUrl para "${att.name}".`);
        }
        let offset = 0;
        while (offset <
            totalBytes) {
            const chunkEnd = Math.min(offset +
                GRAPH_UPLOAD_CHUNK_SIZE, totalBytes);
            ...
            const response = await fetch(uploadUrl, {
                ...
                headers: {
                    /*
                     * Outlook espera bytes crudos
                     * durante la upload session.
                     */
            ...
            const responseBody = await response
                ...
                .catch(() => null);
            if (!response.ok) {
                throw new Error(`Error subiendo "${att.name}" a Graph. HTTP ${response.status} ${response.statusText}. ${JSON.stringify(responseBody)}`);
            }
            /*
             * Si Graph devuelve nextExpectedRanges,
             * significa que la upload session sigue abierta
             * y aún faltan bytes por enviar.
             *
             * Algunos endpoints/tenants pueden responder 200
             * en lugar de 202 durante este proceso.
             */
            const nextExpectedRanges = Array.isArray(responseBody?.nextExpectedRanges)
                ? responseBody.nextExpectedRanges
                : [];
            if (nextExpectedRanges.length >
                0) {
                const nextRange = String(nextExpectedRanges[0]);
                const nextOffset = Number(nextRange
                    .split("-")[0]);
                offset =
            ...
            /*
             * Si ya no hay nextExpectedRanges,
             * Graph considera terminada la carga.
             */
            if (response.status ===
                200 ||
                response.status ===
            ...
            throw new Error(`Graph devolvió una respuesta inesperada al subir "${att.name}". HTTP ${response.status}. ${JSON.stringify(responseBody)}`);
        }
        throw new Error(`La upload session terminó sin confirmar el adjunto "${att.name}".`);
    }
    async verifyDraftAttachments(client, draftId, expectedAttachments) {
        if (expectedAttachments.length ===
            0) {
            return;
        }
        const response = await client
            ...
            .top(100)
            .get();
        const actualAttachments = response?.value ??
            [];
        const missingAttachments = expectedAttachments.filter((expected) => {
            const expectedSize = this.getGraphAttachmentSize(expected.contentBytes);
            return !actualAttachments.some((actual) => String(actual.name ??
                "") ===
                expected.name &&
                Number(actual.size ??
                    0) >
        ...
        if (missingAttachments.length >
            0) {
            throw new Error(`Graph no agregó correctamente al draft los siguientes adjuntos: ${missingAttachments
    ...
    async sendReplyEmail(params) {
        ...
        const attachments = params.attachments ??
            [];
        const LARGE_THRESHOLD = Number(process.env
            .GRAPH_ATTACHMENT_UPLOAD_THRESHOLD_BYTES ??
            GRAPH_SMALL_ATTACHMENT_LIMIT_BYTES);
        const hasLarge = attachments.some((attachment) => this.getGraphAttachmentSize(attachment.contentBytes) >=
            LARGE_THRESHOLD);
        if (!hasLarge) {
            await client
        ...
        for (const att of attachments) {
            const size = this.getGraphAttachmentSize(att.contentBytes);
            if (size <
                LARGE_THRESHOLD) {
                await client
                    .api(`/users/${this.supportEmail}/messages/${draftId}/attachments`)
            ...
            await this
                .uploadLargeAttachmentToDraft(client, draftId, att);
        }
        // 3) send draft
    ...
    async replyToGraphMessage(params) {
        ...
        const draftId = draft?.id ??
            null;
        if (!draftId) {
            throw new Error("Microsoft Graph no devolvió ID para el draft de respuesta.");
        }
        /* ======================================================
           2. ACTUALIZAR BODY / TO / CC
        ====================================================== */
        await client
        ...
        /* ======================================================
           4. VERIFICAR ADJUNTOS ANTES DE ENVIAR
        ====================================================== */
        await this.verifyDraftAttachments(client, draftId, attachments);
        /* ======================================================
           5. OBTENER INTERNET MESSAGE ID
        ====================================================== */
        ...
        await client
            ...
            .post({});
        console.log("✅ Reply real enviado sobre el mensaje original con adjuntos verificados.");
        return {
            graphMessageId: draftWithIds?.id ??
                draftId,
            internetMessageId: draftWithIds?.internetMessageId ??
                null,
        };
    }
