...
export async function getTicketKpis(req, res) {
    try {
        ...
        const frtDurations = frtTickets.map(t => t.firstResponseAt.getTime() - t.createdAt.getTime());
        const avgFirstResponseMs = frtDurations.length > 0
        ...
        const resolutionDurations = resolvedTickets.map(t => t.resolvedAt.getTime() - t.createdAt.getTime());
...
export async function getTicketKpisByAgent(req, res) {
    try {
        const empresaId = req.query.empresaId
            ? Number(req.query.empresaId)
            : undefined;
        ...
        for (const t of tickets) {
            ...
            if (t.firstResponseAt) {
                agent.frtSamples.push(t.firstResponseAt.getTime() - t.createdAt.getTime());
            }
            if (t.resolvedAt) {
                agent.resolutionSamples.push(t.resolvedAt.getTime() - t.createdAt.getTime());
            }
        }
        const kpis = Array.from(agentsMap.values()).map(agent => ({
            ...
            avgFirstResponseMinutes: agent.frtSamples.length > 0
                ? Math.round(agent.frtSamples.reduce((a, b) => a + b, 0) /
                    agent.frtSamples.length /
                    1000 /
                    60)
                : 0,
            avgResolutionMinutes: agent.resolutionSamples.length > 0
                ? Math.round(agent.resolutionSamples.reduce((a, b) => a + b, 0) /
...
//# sourceMappingURL=ticketera-kpis.controller.js.map