...
import { anotarDocumentosCobranza, } from "../../service/baseapi/cobranza/cobranza-estado.service.js";
...
========================================================= */
function getTipoDocBaseApi(doc) {
    return String(doc?.["Tipo Doc"] ??
        doc?.tipoDoc ??
        doc?.tipoDTE ??
        "").trim();
}
function getFolioBaseApi(doc) {
...
function getEmpresaDocumentoBaseApi(doc) {
    const value = String(doc?.empresaOrigen ??
        doc?.empresaKey ??
        doc?.empresa ??
        "")
        .trim()
...
function getFacturaEnvioKey(params) {
    return [
        String(params.empresaKey ??
            "")
            .trim()
            .toLowerCase(),
        String(params.tipoDoc ??
            "")
            .trim(),
        String(params.folio ??
            "")
            .trim(),
        normalizeRut(params.rutContraparte),
    ].join("|");
}
async function anotarDocumentosEnvioFactura(documentos) {
    ...
    const empresas = [
        ...new Set(documentos
            .map((doc) => getEmpresaDocumentoBaseApi(doc))
            .filter((value) => value !==
            null)),
    ];
    const tiposDoc = [
        ...new Set(documentos
            .map((doc) => getTipoDocBaseApi(doc))
            .filter(Boolean)),
    ];
    const folios = [
        ...new Set(documentos
            .map((doc) => getFolioBaseApi(doc))
            .filter(Boolean)),
    ];
    const ruts = [
        ...new Set(documentos
            .map((doc) => getRutDocumentoBaseApi(doc))
            .filter(Boolean)),
    ];
    if (empresas.length ===
        0 ||
     ...
     * =====================================================
     */
    const enviosPorFactura = new Map();
    for (const envio of envios) {
        const key = getFacturaEnvioKey({
            empresaKey: envio
                .empresaKey,
            tipoDoc: envio
                .tipoDoc,
        ...
        const actual = enviosPorFactura.get(key) ??
            [];
        actual.push(envio);
        enviosPorFactura.set(key, actual);
    }
    /*
     * =====================================================
     ...
     */
    return documentos.map((doc) => {
        const empresaKey = getEmpresaDocumentoBaseApi(doc);
        const tipoDoc = getTipoDocBaseApi(doc);
        const folio = getFolioBaseApi(doc);
        const rutContraparte = getRutDocumentoBaseApi(doc);
        if (!empresaKey ||
            !tipoDoc ||
            !folio ||
            !rutContraparte) {
        ...
        const registros = enviosPorFactura.get(key) ??
            [];
        if (registros.length ===
            0) {
            return {
        ...
        const enviados = registros.filter((item) => item.estado ===
            "ENVIADO" ||
            Boolean(item.enviadoAt)).length;
        const pendientes = registros.filter((item) => item.estado ===
            "PENDIENTE").length;
        const procesando = registros.filter((item) => item.estado ===
            "PROCESANDO").length;
        const errores = registros.filter((item) => item.estado ===
            "ERROR").length;
        const cancelados = registros.filter((item) => item.estado ===
            "CANCELADO").length;
        let estado = "PARCIAL";
        if (enviados ===
            registros.length) {
        ...
        const fechasEnvio = registros
            .map((item) => item
            .enviadoAt)
            .filter((value) => value !==
            null);
        const ultimoEnvioAt = fechasEnvio.length >
            0
            ? new Date(Math.max(...fechasEnvio.map((fecha) => fecha
                .getTime())))
            : null;
        const fechasIntento = registros
            .map((item) => item
            .ultimoIntentoAt)
            .filter((value) => value !==
            null);
        const ultimoIntentoAt = fechasIntento.length >
            0
            ? new Date(Math.max(...fechasIntento.map((fecha) => fecha
                .getTime())))
            : null;
        const ultimoConAsunto = [
            ...
            .reverse()
            .find((item) => Boolean(item.asunto));
        return {
            ...doc,
            facturaEnvio: {
                tieneRegistro: true,
                estado,
                totalDestinatarios: registros.length,
                enviados,
                pendientes,
                procesando,
                errores,
                ...
                ultimoEnvioAt,
                ultimoIntentoAt,
                asunto: ultimoConAsunto
                    ?.asunto ??
                    null,
                destinatarios: registros.map((item) => ({
                    id: item.id,
                    nombre: item
                        .nombreDestino,
                    email: item
...
export async function getVentasRcvBaseApi(req, res) {
    try {
        ...
        const documentosConCobranza = await anotarDocumentosCobranza(data.data?.datos ?? [], "ventas");
        const documentosAnotados = await anotarDocumentosEnvioFactura(documentosConCobranza);
        data = {
            ...data,
            detalleVentas: documentosAnotados,
            ventas: documentosAnotados,
            documentos: documentosAnotados,
            total: documentosAnotados.length,
            data: {
                ...(data.data || {}),
                datos: documentosAnotados,
                totalRegistros: documentosAnotados.length,
            },
        };
        res.json({
            ok: true,
...
export async function getComprasRcvBaseApi(req, res) {
    try {
        ...
        // Anotar estadoPago en cada documento: CONFIRMADA | VENCIDA | PENDIENTE
        const documentosAnotados = await anotarDocumentosCobranza(data.data?.datos ?? [], "compras");
        data = {
            ...data,
            detalleCompras: documentosAnotados,
            compras: documentosAnotados,
            documentos: documentosAnotados,
            total: documentosAnotados.length,
            data: {
                ...(data.data || {}),
