...
export async function getEmpresas(req, res) {
    try {
        ...
        const empresasConStats = empresas.map((empresa) => {
            ...
            const totalEquipos = empresa.solicitantes.reduce((acc, sol) => acc + (sol.equipos?.length || 0), 0);
            ...
            // Visitas pendientes
            const visitasPendientes = empresa.visitas.filter((v) => v.status === "PENDIENTE").length;
            // Trabajos pendientes
            const trabajosPendientes = empresa.detalleTrabajos.filter((t) => t.estado === "pendiente" || t.estado === "PENDIENTE").length;
            return {
...
export async function getEmpresasStats(req, res) {
    try {
        ...
        const statsTotales = empresas.reduce((acc, empresa) => {
            const equiposEmpresa = empresa.solicitantes.reduce((sum, sol) => sum + (sol.equipos?.length || 0), 0);
            const ticketsAbiertosEmpresa = empresa.tickets.filter((t) => t.status !== 5).length;
            const visitasPendientesEmpresa = empresa.visitas.filter((v) => v.status === "PENDIENTE").length;
            const trabajosPendientesEmpresa = empresa.detalleTrabajos.filter((t) => t.estado === "pendiente" || t.estado === "PENDIENTE").length;
            return {
