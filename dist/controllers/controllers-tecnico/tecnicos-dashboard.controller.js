...
function diffMinutos(inicio, fin) {
    ...
    if (!Number.isFinite(ini) || !Number.isFinite(end))
        return 0;
    if (end <= ini)
        return 0;
    return Math.round((end - ini) / 60000);
}
function ensureMes(row, mes) {
...
/* ───────────────────────── Controller ───────────────────────── */
export const getTecnicosHorasHombreDashboard = async (req, res) => {
    try {
        ...
        const tecnicoIds = Array.from(new Set([
        ...
        const tecnicoMap = new Map(tecnicos.map((t) => [t.id_tecnico, t.nombre]));
        ...
        for (const r of remotas) {
            if (!r.empresaId || !r.tecnicoId || !r.inicio)
                continue;
        ...
        for (const t of tickets) {
            if (!t.empresaId || !t.assigneeId)
                continue;
            const fechaCierre = t.closedAt ?? t.resolvedAt ?? t.updatedAt;
            if (!fechaCierre)
                continue;
            if (fechaCierre < desde || fechaCierre >= hasta)
                continue;
        ...
        const rows = Array.from(rowsMap.values())
            ...
            return {
                ...
                meses: Array.from(row.meses.values())
                    ...
                    horasTotal: Number(((m.minutosVisitas + m.minutosRemotas) / 60).toFixed(2)),
                }))
        ...
        const resumenPorTecnico = Array.from(resumenPorTecnicoMap.values())
            ...
            .sort((a, b) => b.totalHorasHombre - a.totalHorasHombre);
        /* ───────────────────────── KPIs generales ───────────────────────── */
        const totalMinutosVisitas = rows.reduce((acc, r) => acc + r.totalMinutosVisitas, 0);
        const totalMinutosRemotas = rows.reduce((acc, r) => acc + r.totalMinutosRemotas, 0);
        ...
        return res.json({
            ...
            data: {
                ...
                kpis: {
                    ...
                    totalHorasHombre: Number(((totalMinutosVisitas + totalMinutosRemotas) / 60).toFixed(2)),
                    totalVisitas,
                    // Alias para compatibilidad
...
//# sourceMappingURL=tecnicos-dashboard.controller.js.map