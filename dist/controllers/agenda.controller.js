...
/* ================== Handlers ================== */
// POST /agenda/generar
export async function generarMalla(req, res) {
    try {
...
export async function getAgendaDesdeOutlookController(req, res) {
    try {
...
export async function syncAgendaOutlook(req, res) {
    try {
...
// DELETE /agenda/:id
export async function eliminarVisita(req, res) {
    try {
        const id = Number(req.params.id);
        if (isNaN(id))
            return res.status(400).json({ error: "ID inválido" });
...
// POST /agenda/manual
export async function crearVisitaManual(req, res) {
    try {
...
export async function reprogramarTecnicos(req, res) {
    try {
        const id = Number(req.params.id);
        if (isNaN(id))
            return res.status(400).json({ error: "ID inválido" });
...
export async function enviarNotaAgenda(req, res) {
    try {
        const id = Number(req.params.id);
        if (isNaN(id))
