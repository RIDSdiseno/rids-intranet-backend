...
====================================================== */
function resolveSharepointPathReporte(empresa) {
...
====================================================== */
export async function exportReportesForSharepoint(req, res) {
    try {
        /* =====================
           Mes (YYYY-MM)
        ===================== */
        const now = new Date();
        const mes = typeof req.body?.month === "string" &&
        ...
        for (const empresa of empresas) {
            const sharepointPath = resolveSharepointPathReporte(empresa.nombre);
            if (!sharepointPath) {
                console.warn(`⚠️ Empresa sin ruta SharePoint (Reporte): ${empresa.nombre}`);
                continue;
            }
            // PASO 1
            const data = await buildReporteEmpresaData(empresa.id_empresa, mes);
            // PASO 2
            const buffer = await buildReporteEmpresaDocx(data);
            archivos.push({
                empresa: empresa.nombre,
                sharepointPath,
                fileName: `Reporte_${normalizeEmpresa(empresa.nombre)}_${mes}_${timestamp}.docx`,
...
//# sourceMappingURL=reportes-export.controller.js.map