 ...
 */
export function redondearDinero(value) {
 ...
 */
export function normalizarDescuento(value) {
    const parsed = parseDecimalInput(value, 0);
    return redondearDinero(Math.min(100, Math.max(0, parsed)));
}
/**
 ...
 */
export function normalizarGanancia(value) {
    const parsed = parseDecimalInput(value, 0);
    return redondearDinero(Math.max(0, parsed));
}
/**
...
export function normalizarCantidad(value) {
    const parsed = parseDecimalInput(value, 1);
 ...
 */
export function calcularTotalesCotizacionBackend(items) {
    let subtotalBruto = 0;
    ...
    for (const item of items) {
        ...
        const precio = redondearDinero(parseDecimalInput(item.precioOriginalCLP ??
            item.precio, 0));
    ...
    for (const item of items) {
        ...
        const precio = redondearDinero(parseDecimalInput(item.precioOriginalCLP ??
            item.precio, 0));
        const cantidad = normalizarCantidad(item.cantidad);
        const porcentaje = normalizarDescuento(item.porcentaje);
        const base = precio * cantidad;
        const descuento = Boolean(item.tieneDescuento) &&
            porcentaje > 0
            ? base *
                (porcentaje / 100)
            : 0;
        const neto = Math.max(0, base - descuento);
    ...
    descuentoGlobal = Math.min(descuentoGlobal, subtotalAntesGlobal);
    const subtotal = Math.max(0, subtotalAntesGlobal -
        descuentoGlobal);
    /*
     * Reducir proporcionalmente el IVA cuando
     * se aplica un descuento global.
     */
    const factorGlobal = subtotalAntesGlobal > 0
        ? subtotal /
            subtotalAntesGlobal
        : 0;
    const iva = ivaAntesGlobal *
        factorGlobal;
    const descuentos = descuentosIndividuales +
        descuentoGlobal;
    const total = subtotal + iva;
    return {
        subtotalBruto: redondearDinero(subtotalBruto),
        descuentos: redondearDinero(descuentos),
        subtotal: redondearDinero(subtotal),
        iva: redondearDinero(iva),
        total: redondearDinero(total),
    };
}
//# sourceMappingURL=cotizaciones-numeros.js.map