...
import { calcularAnioPcDesdeSerial, normalizarAnioPc, } from "../utils/equipos/anio-pc.util.js";
...
const PROPIEDADES_EQUIPO = ["Empresa", "Personal", "Externo"];
/* ================== Schemas ================== */
...
function normalizarPropiedadEquipo(value) {
    const propiedad = String(value ?? "Empresa").trim();
    if (PROPIEDADES_EQUIPO.includes(propiedad)) {
        return propiedad;
    }
    return "Empresa";
}
function normalizarPropietarioExterno(propiedad, value) {
...
function normalizarBusquedaTipoEquipo(value) {
    return value
        .trim()
        .toLowerCase()
        .normalize("NFD")
...
function obtenerTiposEquipoDesdeBusqueda(search) {
    ...
    return opciones
        .filter((item) => item.palabras.some((palabra) => {
...
function flattenRow(e) {
    ...
    return {
        ...
        solicitanteEmpresaId: e.solicitante?.empresaId ??
            e.solicitante?.empresa?.id_empresa ??
            null,
        solicitanteEmpresa: e.solicitante?.empresa?.nombre ??
            null,
...
export async function listEquipos(req, res) {
  ...
  `;
     ...
     */
        andConditions.push(buildEquipoEmpresaActivaWhere());
        /* =========================
           Restricción por rol CLIENTE
        ========================= */
        if (user?.rol === "CLIENTE") {
            const empresaClienteId = Number(user.empresaId);
            andConditions.push({
                OR: [
                    {
            ...
            try {
                ...
                if (data.empresaId) {
                    ...
                    if (!empresa.isActive) {
                        errors.push({
                            serial: data.serial,
                            error: "La empresa está inactiva y no puede recibir equipos.",
                        });
                        continue;
                    }
                }
                ...
                if (data.idSolicitante) {
                    ...
                    if (!solicitante ||
                        ...
                        errors.push({
                            serial: data.serial,
                            error: "El solicitante o su empresa se encuentran inactivos.",
                        });
                        continue;
                    }
                }
                ...
                if (data.idSolicitante !== undefined &&
                    ...
                    if (!solicitante ||
                        ...
                        !solicitante.empresa.isActive) {
                        return res.status(409).json({
                            code: "SOLICITANTE_O_EMPRESA_INACTIVA",
                            error: "No se puede asignar el equipo a un solicitante o empresa inactivos.",
                        });
                    }
                }
                ...
                const anioPcManual = bodyTieneAnioPc
                    ...
                    : null;
                const calculoAnioPc = calcularAnioPcDesdeSerial(data.serial, data.marca, data.modelo);
                ...
                const propietarioExternoNormalizado = normalizarPropietarioExterno(propiedadNormalizada, data.propietarioExterno);
...
export async function getEquipoById(req, res) {
    try {
        ...
        const empresaEquipo = equipo.empresa ??
...
export async function updateEquipo(req, res) {
    try {
        ...
        const empresaIdFinal = data.empresaId ??
        ...
        const propiedadFinal = normalizarPropiedadEquipo(data.propiedad ?? equipoActual.propiedad);
        const propietarioExternoFinal = normalizarPropietarioExterno(propiedadFinal, data.propietarioExterno ?? equipoActual.propietarioExterno);
        ...
        if (data.idSolicitante !== undefined &&
            ...
            if (!solicitante.isActive ||
                ...
                return res.status(409).json({
                    ...
                    error: "El solicitante seleccionado o su empresa se encuentran inactivos.",
                });
            }
            if (solicitante.empresaId !== empresaIdFinal) {
                return res.status(409).json({
                    code: "SOLICITANTE_EMPRESA_NO_COINCIDE",
                    error: "El solicitante seleccionado no pertenece a la empresa del equipo.",
                });
            }
            solicitanteIdFinal = solicitante.id_solicitante;
        }
        else if (data.idSolicitante === null ||
             ...
             * a la empresa final.
             */
            const solicitanteActualId = equipoActual.idSolicitante;
            if (solicitanteActualId) {
                const solicitanteActual = await prisma.solicitante.findUnique({
                    where: {
                        id_solicitante: solicitanteActualId,
                    },
                    select: {
                        id_solicitante: true,
                        empresaId: true,
                ...
                const actualValido = solicitanteActual &&
        ...
        const bodyTieneAnioPc = Object.prototype.hasOwnProperty.call(data, "anioPc");
        let anioPcFinal = equipoActual.anioPc;
        let anioPcOrigenFinal = equipoActual.anioPcOrigen ?? "NO_DETERMINADO";
        const cambioDatosCalculo = equipoData.serial !== undefined ||
        ...
        if (bodyTieneAnioPc) {
            ...
            const calculoAnioPc = calcularAnioPcDesdeSerial(serialFinal, marcaFinal, modeloFinal, procesadorFinal);
...
function normalizeAuditChanges(value) {
    ...
    for (const [field, change] of Object.entries(value)) {
        ...
        const tieneBefore = Object.prototype.hasOwnProperty.call(change, "before");
        const tieneAfter = Object.prototype.hasOwnProperty.call(change, "after");
...
function filterEquipoHistoryChanges(value) {
    ...
    for (const [field, change] of Object.entries(changes)) {
        ...
        if (JSON.stringify(change.before ?? null) ===
...
export async function getEquipoHistorial(req, res) {
    try {
        ...
        const auditItems = [...logsEquipo, ...logsDetalle]
            ...
            const normalizedChanges = normalizeAuditChanges(log.changes);
            /*
            ...
            */
            ...
            const ocultarHistorial = !filteredChanges ||
        ...
        const existeCreateAudit = auditItems.some((item) => item.entity === "Equipo" &&
        ...
        const createFallbackItem = !existeCreateAudit
            ? {
                ...
                description: "Equipo creado según la fecha original del registro. No existe una auditoría histórica para identificar al autor ni reconstruir todos sus datos iniciales.",
                changes: {
                    id_equipo: {
                        before: null,
        ...
        const agenteItems = eventosAgente.map((ev) => ({
            ...
            action: ev.tipo === "INVENTORY_CREATED"
                ? "CREATE"
...
export async function recalcularAnioPcEquipos(req, res) {
    try {
        ...
        for (const equipo of equipos) {
            const calculo = calcularAnioPcDesdeSerial(equipo.serial, equipo.marca, equipo.modelo, equipo.procesador);
            const anioPcNuevo = calculo.anioPc;
            const anioPcOrigenNuevo = calculo.anioPcOrigen;
            const cambio = equipo.anioPc !== anioPcNuevo ||
