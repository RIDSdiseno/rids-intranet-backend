...
export async function buildReporteEmpresaDocx(data) {
    ...
    }), new Paragraph(`Visitas realizadas: ${data.kpis.visitas.count}`), new Paragraph(`Equipos registrados: ${data.kpis.equipos.count}`), new Paragraph(`Tickets generados: ${data.kpis.tickets.total}`), new Paragraph({ text: "", spacing: { after: 300 } }));
    /* =====================
       Tabla KPIs
    ===================== */
        ...
        rows: [
            ...
            new TableRow({
                children: [
                    ...
                    new TableCell({
                        children: [
                            new Paragraph(`${Math.round(data.kpis.visitas.avgMs / 60000)} min`),
                        ],
                    }),
                ],
            }),
    ...
    if (data.visitasPorTipo.length > 0) {
        ...
        const chartPng = await renderVisitasPorTipoChart(data.visitasPorTipo);
        children.push(new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [
                new ImageRun({
