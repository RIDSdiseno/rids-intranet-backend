...
function subjectForDomain(domain) {
    ...
    if (!domain) {
        if (!fallback)
            throw new Error("Falta GOOGLE_IMPERSONATED_ADMIN en .env");
        return fallback;
    }
    // Parsea "alianz.cl:admin@alianz.cl,fijacionesprocret.cl:admin@fijacionesprocret.cl"
    const map = raw.split(",").reduce((acc, pair) => {
        const [d, s] = pair.split(":").map(x => x?.trim()).filter(Boolean);
        if (d && s)
            acc[d.toLowerCase()] = s;
        return acc;
    }, {});
    ...
    if (!subject) {
        throw new Error(`No hay subject para el dominio "${domain}". Define GOOGLE_IMPERSONATED_MAP o GOOGLE_IMPERSONATED_ADMIN.`);
    }
    return subject;
}
/** Autenticación por Service Account + impersonación del admin (por dominio) */
export function getDirectoryClient(domain) {
...
export async function listAllUsers(domain) {
    if (!domain)
        throw new Error("domain requerido");
...
//# sourceMappingURL=googleDirectory.js.map